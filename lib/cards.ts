...
export type SavedCard = {
  ...
  name: string;
  exp: string; // "MM/YY"
  cvv?: string; // chỉ để tương thích dữ liệu/type cũ; không lưu CVV thật
...
export function saveCard(card: Omit<SavedCard, "savedAt">): void {
  try {
    const { cvv: _ignoredCvv, ...safeCard } = card;
    localStorage.setItem(KEY, JSON.stringify({ ...safeCard, savedAt: Date.now() }));
  } catch {
