...
export interface Store {
  ...
  name: string;
  address: string;
  /** Mã terminal Co.op Online. Dùng làm khoá checkout: một đơn Co.op chỉ có một terminal. */
  terminalCode?: string;
...
export interface Product {
  ...
  variantName1?: string;
  variantName2?: string;
  /** Danh mục trên trang chủ PNJ (dùng khi chọn chuỗi pnj). VD: "Nhẫn Cười", "Nhẫn Kim cương" */
  pnjCategory?: string;
  /** Size mặc định cần chọn khi mua sản phẩm này tại PNJ. VD: "17" */
  pnjSize?: string;
  /** URL sản phẩm trực tiếp trên pnj.com.vn — agent dùng để navigate thẳng vào trang SP. */
  pnjUrl?: string;
  /** Ngày sinh mặc định điền vào form PNJ (YYYY-MM-DD). Agent dùng để tự điền trường "Ngày sinh". */
  pnjDob?: string;
}
