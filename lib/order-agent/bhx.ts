      ...
      ws.send(
        JSON.stringify({
          ...
          payload: {
            ...
            cardInfo: ctx.demoPayMethod === "card" ? { number: ctx.cardNum, exp: ctx.cardExp, cvv: ctx.cardCvv } : null,
            slot: ctx.getSlotKey(),
          },
        }),
      );
  ...
  ws.onmessage = (event) => {
    try {
      ...
      if (message.type === "status") {
        ...
        console.log("BHX order code:", message.content);
        ctx.setOrderCode(message.content);
      } else if (message.type === "order_success") {
        console.log("BHX order success:", message.content);
        ctx.setBhxBusy(false);
        if (message.content) {
          ctx.setBhxQR(message.content);
        }
        ctx.setStepIndex(7);
        if (ctx.demoPayMethod === 'cod' || ctx.demoPayMethod === 'card') {
          ctx.setPhase("done");
        }
