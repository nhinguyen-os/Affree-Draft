...
export interface InMemoryOrderSessionStore {
  ...
  saveInteractionFrame(id: string, interactionFrameBase64: string, interactionFrameContentType?: string): OrderSessionPrivateState | null;
...
export function createInMemoryOrderSessionStore(ttlMs = DEFAULT_TTL_MS): InMemoryOrderSessionStore {
  ...
  return {
      ...
      const next: OrderSessionPrivateState = {
        ...
        private: {
          ...
          ...(popupState
            ? {}
            : {
                interactionFrameBase64: undefined,
                interactionFrameContentType: undefined,
    ...
    saveInteractionFrame(id, interactionFrameBase64, interactionFrameContentType = "image/jpeg") {
      const current = get(id);
      if (!current) return null;
      const next: OrderSessionPrivateState = {
        ...current,
        ...
        private: {
          ...current.private,
          interactionFrameBase64,
          interactionFrameContentType,
          popupFrameBase64: interactionFrameBase64,
          popupFrameContentType: interactionFrameContentType,
        },
      };
      sessions.set(id, next);
      return next;
    ...
    savePopupFrame(id, popupFrameBase64, popupFrameContentType = "image/jpeg") {
      return this.saveInteractionFrame(id, popupFrameBase64, popupFrameContentType);
    },
    clearPopupState(id) {
      const current = get(id);
      if (!current) return null;
