...
/** Key chuỗi (vd "coop", "bhx") để CobrowseSession hiện logo thật qua ChainBadge. */
function storeChainKey(offer: RankedOffer): string | undefined {
  for (const cand of [offer.store.chain, offer.store.id, offer.store.name]) {
    const k = chainKey(cand);
    if (SOURCE_META[k]) return k;
  }
  return undefined;
}
...
function storeDomain(offer: RankedOffer): string {
  // Dùng resolveMeta (chuẩn hoá chain/id/name, vd "COOP_00019" → coop) để lấy ĐÚNG home thật
  // của nhà bán; trước đây tra SOURCE_META[offer.store.chain] thô nên store-id dạng mã rơi về
  // fallback "cua-hang.vn". Ưu tiên website riêng của cửa hàng nếu có.
  const home = offer.store.website || resolveMeta(offer)?.home || "";
  try {
    // Giữ nguyên host thật của nhà bán (kèm "www" nếu có) + trang giỏ/checkout.
