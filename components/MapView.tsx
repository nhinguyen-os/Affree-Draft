...
import { MapContainer, TileLayer, Popup, useMap, CircleMarker, Circle, AttributionControl } from "react-leaflet";
...
export default function MapView({
  ...
  useEffect(() => {
    if (activeStore) {
      const stillExists = markers.some((m) => m.store.id === activeStore.id);
      if (!stillExists) {
        setActiveStore(null);
      }
    }
  ...
  const activeMarker = markers.find((m) => m.store.id === activeStore?.id);
  const activeStorePrice = activeMarker?.price;
  const activeStoreInStock = activeMarker?.inStock;
  const activeStoreCheapest = activeMarker?.cheapest;
  ...
  return (
    <div style={{ position: "relative", height: "100%", width: "100%" }}>
      ...
      >
        ...
        {activeStore && (
          <Popup
            position={[activeStore.lat as number, activeStore.lng as number]}
            eventHandlers={{
              remove: () => setActiveStore(null),
            }}
          >
            <div style={{ minWidth: 160 }}>
              <div style={{ fontWeight: 700 }}>{chainLabel(activeStore.chain)}</div>
              <div>{activeStore.name}</div>
              <div style={{ color: "#666", fontSize: 12 }}>{activeStore.address}</div>
              {activeStorePrice != null && (
                <div style={{ marginTop: 4, fontWeight: 700, color: activeStoreCheapest ? "#16a34a" : "#111" }}>
                  {activeStoreInStock ? formatMoney(activeStorePrice, storeCurrency(activeStore.id)) : t("Hết hàng")}
                  {activeStoreCheapest && activeStoreInStock ? ` · ${t("Rẻ nhất")}` : ""}
                </div>
              )}
              {onBuy && (
                <button
                  type="button"
                  onClick={() => onBuy(activeStore)}
                  style={{
                    marginTop: 8,
                    width: "100%",
                    display: "flex",
