...
export default function CoopOrderAgentModal({
  offer,
  alternatives = [],
  geoAddr,
  geoLat,
  ...
  defaultQty?: number;
  /** Các dòng trong giỏ thuộc cùng một terminal Co.op. Có dữ liệu này thì tạo 1 đơn nhiều sản phẩm. */
  sameStoreCartItems?: CartItem[];
  /** Khi mở từ giỏ, không đổi sang terminal gần nhất: mỗi checkout phải giữ đúng cửa hàng đã chọn. */
  lockedTerminalCode?: string;
  initialPayMethod?: DemoPayMethod | null;
  initialSlot?: string;
  /** Dùng khi mở từ CartModal: form đã đủ thông tin, tự bấm bước kết nối Co.op để tránh phải click lần 2. */
  autoStart?: boolean;
  onClose: () => void;
  ...
  const isTXNNReal = process.env.NEXT_PUBLIC_TXNN_REAL === "true" && activeOffer.store.chain === "tuoixanhnhanhngon";
  const isBHXReal = process.env.NEXT_PUBLIC_BHX_REAL === "true" && activeOffer.store.chain === "bhx";
  ...
  const coopOrderLines = useMemo(() => {
    const sameStore = sameStoreCartItems
      .map((item) => ({
        product: item.product,
        offer: item.offer,
        qty: Math.max(1, Math.floor(item.qty || 1)),
      }));
    if (sameStore.length) return sameStore;
    return [{ product: activeOffer.product, offer: activeOffer, qty }];
  }, [activeOffer, qty, sameStoreCartItems]);
  const isMultiCoopOrder = isCoopReal && coopOrderLines.length > 1;
  const coopOrderTotal = coopOrderLines.reduce((sum, line) => sum + line.offer.price * line.qty, 0);
  const coopOrderSummaryName =
    coopOrderLines.length > 1
      ? `${coopOrderLines[0].product.name} + ${coopOrderLines.length - 1} sản phẩm`
      : activeOffer.product.name;
  const [slot, setSlot] = useState(initialSlot && ACCEPTED_PROTOTYPE_SLOTS.includes(initialSlot) ? initialSlot : SLOTS[0]);
  ...
  useEffect(() => {
    if (!isPrototypeCoopSlotPast(slot)) return;
    const pool = CART_PROTOTYPE_SLOTS.includes(slot) ? CART_PROTOTYPE_SLOTS : SLOTS;
    const next = pool.find((item) => !isPrototypeCoopSlotPast(item));
  ...
  const [demoPayMethod, setDemoPayMethod] = useState<DemoPayMethod | null>(initialPayMethod);
  const [cardNum, setCardNum] = useState("");
  // Tên chủ thẻ — ô riêng, ĐỒNG BỘ với form giỏ hàng / Mua ngay.
  ...
  const savedCardNumberDigits = savedCard?.number.replace(/\D/g, "") || "";
  const cardNumberDigits = (usingSavedCard ? savedCardNumberDigits : cardNum.replace(/\D/g, ""));
  const cardExpiry = usingSavedCard && savedCard ? savedCard.exp : cardExp;
  const cardCvvDigits = cardCvv.replace(/\D/g, "");
  const cardEmailRequired = demoPayMethod === "card";
  // Thẻ "sẵn sàng" — luôn cần CVV nhập mới cho phiên hiện tại; CVV không được lưu.
  const newCardReady = cardNum.replace(/\s/g, "").length >= 12 && /^\d{2}\/\d{2}$/.test(cardExp) && cardCvvDigits.length >= 3;
  const savedCardReady = usingSavedCard && savedCardNumberDigits.length >= 12 && /^\d{2}\/\d{2}$/.test(savedCard?.exp || "") && cardCvvDigits.length >= 3;
  ...
  const coopPaymentCardPayload = () => {
    if (demoPayMethod !== "card" || !cardReady) return undefined;
    ...
    return {
      ...
      expiryYear,
      cvv: cardCvvDigits,
      cardholderName: toVnpayAscii(usingSavedCard && savedCard?.name ? savedCard.name : name).toUpperCase(),
      email: email.trim(),
      billingAddress: toVnpayAscii(address),
  ...
  const [coopPaymentVerifying, setCoopPaymentVerifying] = useState(false);
  ...
  const steps: Step[] = useMemo(() => {
    ...
    s.push({
      kind: "auto",
      label: coopOrderLines.length > 1
        ? t("Thêm {count} sản phẩm vào giỏ Co.op…", { count: coopOrderLines.length })
        : t('Thêm "{name}" vào giỏ (SL {qty})…', { name: activeOffer.product.name, qty }),
    });
  ...
  const total = isCoopReal ? coopOrderTotal : activeOffer.price * qty;
  ...
  const coopDeliveryDates = (() => {
    ...
    const hasPreferredSlots = Boolean(coopResult?.deliveryCheck?.availableSlotsByDate?.[preferredDate]?.length);
    const hasOnlyGenericSlots = !dates.length && Boolean(coopResult?.deliveryCheck?.availableTimeSlots?.length);
    return hasPreferredSlots && preferredDate >= todayInput && !dates.includes(preferredDate)
      ? [preferredDate, ...dates].sort()
      : hasOnlyGenericSlots && preferredDate >= todayInput
        ? [preferredDate]
      : dates;
  })();
  ...
  // GUI Co.op rút gọn chỉ còn ba lựa chọn. Mỗi lựa chọn vẫn giữ nguyên
  // methodCode thật mà API trả về để prepareCheckout/placeOrder dùng đúng logic Dev.
  const coopPaymentMethods = selectCoopPrototypePaymentMethods(
    (coopResult?.paymentCheck?.methods ?? []).filter((method) => method.methodCode),
  );
  ...
  const isCoopPrototypeSlotDisabled = (label: string) => {
    ...
    if (dates.length && !dates.includes(date)) return true;
    const slotsForDate =
  ...
  const activeCoopTerminalCode = activeOffer.store.terminalCode?.trim() || activeOffer.store.id.replace(/^coop-/, "");
  const fixedCoopTerminalCode = lockedTerminalCode?.trim() || "";
  const coopTerminalCode = fixedCoopTerminalCode || coopSelectedTerminalCode || activeCoopTerminalCode;
  ...
  const emailRequired = cardEmailRequired;
  const emailValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim());
  const emailBlocksStart = emailRequired && !emailValid;
  const emailError = emailRequired && email.trim().length > 0 && !emailValid;
  ...
  const coopPayload = () => ({
    ...
    items: coopOrderLines.map((line) => ({
      ...
      productUrl: line.offer.productUrl,
      terminalCode: coopTerminalCode,
    })),
    terminalCode: coopTerminalCode,
  ...
  const applyCoopDeliverySelection = (data: CoopOrderResult) => {
    ...
    let guiSlot = coopRunSlotRef.current || selectedPrototypeSlot;
    let preferredDate = prototypeSlotDate(guiSlot);
    if (dates.length && !dates.includes(preferredDate)) {
      const fallbackSlot = SLOTS.find((item) => {
        const date = prototypeSlotDate(item);
        if (!dates.includes(date)) return false;
        const slots = data.deliveryCheck?.availableSlotsByDate?.[date] ?? [];
        return Boolean(findClosestCoopSlot(getEffectiveCoopDeliverySlots(slots, date), item));
      });
      if (fallbackSlot) {
        guiSlot = fallbackSlot;
        preferredDate = prototypeSlotDate(guiSlot);
        setSlot(fallbackSlot);
        coopRunSlotRef.current = fallbackSlot;
      }
    }
    const slotsForPreferredDate =
      data.deliveryCheck?.availableSlotsByDate?.[preferredDate]?.length
        ...
        : !dates.length && data.deliveryCheck?.availableTimeSlots?.length
          ? data.deliveryCheck.availableTimeSlots
          : [];
    const selectedDate =
      dates.includes(preferredDate) || (!dates.length && slotsForPreferredDate.length)
        ? preferredDate
        : "";
  ...
  const setCoopCartReady = (data: CoopOrderResult) => {
    ...
    const selectedFromPrototype = prototypeMethods.find((method) =>
      demoPayMethod === "qr"
        ? method.name === "QR"
        : demoPayMethod === "card"
          ? method.name === "Thẻ Quốc Tế"
          : method.name === "COD",
    );
    setCoopSelectedPaymentCode(selectedFromPrototype?.methodCode || data.paymentCheck?.selectedMethodCode || "COD");
  ...
  useEffect(() => {
    if (!autoStart || coopAutoStartRef.current) return;
    if (!isCoopReal || phase !== "form" || !canStart || coopBusy) return;
    coopAutoStartRef.current = true;
    void startCoopOrder();
    // startCoopOrder intentionally runs once for cart-origin modal after derived readiness settles.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [autoStart, isCoopReal, phase, canStart, coopBusy]);
  ...
  // Sau khi API tạo giỏ và trả ngày/khung giờ thật, GUI prototype không có nút
  // "Cập nhật checkout" riêng nên tự gửi lịch giao + phương thức đã chọn.
  useEffect(() => {
  ...
  const confirmFinalOrder = async () => {
    ...
    if (isCoopReal) {
      ...
      if (!coopCheckoutPrepared) {
        setCoopError(t("Co.op chưa cập nhật lịch giao và phương thức thanh toán, vui lòng chờ trợ lý AAAI chuẩn bị xong."));
        return;
      }
      setStepIndex((x) => x + 1);
  ...
  const openCoopPaymentQrSnapshot = async (result: CoopOrderResult) => {
    ...
    const ws = new WebSocket(await getAgentWsUrl(wsSessionId, "coop"));
    ...
    ws.onmessage = (event) => {
      ...
      if (message.type === "status" && message.phase === "coop_payment_qr_ready" && message.qrImageBase64) {
        ...
        setCoopPaymentQrBusy(false);
        setCoopPaymentVerifying(false);
        setCoopError(message.error || t("Không lấy được mã QR VNPAY."));
      } else if (message.type === "status" && message.phase === "completed" && !coopCompletionHandledRef.current) {
        window.clearTimeout(timeout);
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (message.type === "status" && message.phase === "completed" && !coopCompletionHandledRef.current) {
            ...
            setOrderCode(code);
            setCoopBrowserVisible(true);
            setCoopBrowserStatus(t("Thanh toán đã hoàn tất. Giữ lại màn hình cuối để bạn kiểm tra."));
            setCoopStep("success");
            setPhase("done");
            setCoopResult((current) => ({
              ...(current ?? {}),
  ...
  const checkCoopAddress = async () => {
    ...
    try {
      ...
      const fixedTerminal = fixedCoopTerminalCode
        ? terminals.find((item) => getCoopTerminalCode(item) === fixedCoopTerminalCode)
        : null;
      if (fixedCoopTerminalCode && !fixedTerminal) {
        throw new Error(t("Cửa hàng Co.op đã chọn hiện không hỗ trợ giao tới địa chỉ này. Vui lòng sửa giỏ hoặc chọn địa chỉ khác."));
      }
      const selectedCode = fixedCoopTerminalCode || data.selectedTerminalCode || getCoopTerminalCode(terminals[0]);
      setCoopTerminals(terminals);
      setCoopSelectedTerminalCode(selectedCode);
      setCoopAddressChecked(true);
      const matchingOffer = fixedCoopTerminalCode
        ? undefined
        : alternatives.find(
            (item) => item.store.chain === "coop" && (item.store.terminalCode || item.store.id.replace(/^coop-/, "")) === selectedCode,
          );
  ...
  // Không render modal wizard riêng của bản Dev. Co.op thật phải dùng đúng phần
  // giao diện prototype chung bên dưới; toàn bộ state/API/agent logic vẫn giữ nguyên.
  if (false && isCoopReal) {
    const coopWizard = [
      { id: "account" as CoopStepId, label: "Thông tin người nhận" },
      { id: "otp" as CoopStepId, label: "Xác thực tài khoản đặt hộ" },
                ...
                <div className="rounded-lg border border-slate-200 bg-slate-50 p-2.5">
                  <div className="flex items-center gap-2.5">
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-emerald-100 text-sm font-bold text-emerald-700">
                      {coopOrderLines.length > 1 ? coopOrderLines.length : qty}
                    </span>
                    <div className="min-w-0">
                      <p className="truncate text-sm font-bold text-slate-900">{coopOrderSummaryName}</p>
                      <p className="mt-0.5 truncate text-xs text-slate-500">
                        {activeOffer.store.name} · {formatMoney(total, storeCurrency(activeOffer.store.id))}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {phase === "running" && coopStep === "review" && (
                  ...
                  >
                    <div className="space-y-1.5 rounded-lg bg-white p-2.5 text-sm">
                      <Row k={t("Món")} v={isMultiCoopOrder ? `${coopOrderSummaryName} (${coopOrderLines.length} món)` : `${activeOffer.product.name} ×${qty}`} />
  ...
  const coopInlineBrowserPanel = isCoopReal && (coopBrowserVisible || coopBrowserFrame) ? (
    <div
      ...
      onMouseDown={focusCoopBrowser}
    >
      <div className="flex items-center justify-between gap-2 bg-slate-900 px-3 py-2 text-xs text-white">
        <span className="truncate">{coopBrowserStatus || t("Màn hình thanh toán VNPAY")}</span>
        <button type="button" onClick={closeCoopBrowser} className="shrink-0 rounded bg-white/10 px-2 py-1 font-semibold hover:bg-white/20">
          {t("Đóng màn hình")}
        </button>
      </div>
      <div
        ref={coopStreamWheelRef}
        className="coop-stream-wheel-capture flex min-h-[260px] items-center justify-center bg-white"
        style={{ aspectRatio: `${coopBrowserSize.width} / ${coopBrowserSize.height}` }}
      >
        {coopBrowserFrame ? (
          <img
            ref={coopStreamImageRef}
            src={coopBrowserFrame}
            alt={t("Màn hình thanh toán VNPAY")}
            className="h-full max-h-[520px] w-full cursor-crosshair object-contain"
            onClick={clickCoopBrowser}
          />
        ) : (
          <div className="flex h-full min-h-[260px] items-center justify-center text-sm text-slate-500">
            <Spinner />
            <span className="ml-2">{t("Đang chờ ảnh màn hình VNPAY từ agent-server…")}</span>
          </div>
        )}
      </div>
      <div className="border-t border-slate-200 bg-slate-50 px-3 py-2 text-[11px] leading-4 text-slate-500">
        {t("Đây là màn hình thật agent đang thao tác. Bạn có thể click vào ảnh để thao tác trực tiếp nếu cần.")}
      </div>
    </div>
  ) : null;
  ...
  return (
    ...
    >
      <div
        className={`relative flex max-h-[92vh] w-full flex-col overflow-hidden rounded-3xl ${coopInlineBrowserPanel ? "max-w-6xl" : "max-w-md"}`}
        onClick={(e) => e.stopPropagation()}
        style={{
      ...
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 py-4">
            ...
            <div className="mb-4 flex items-stretch gap-3">
              ...
              {demoPayMethod === "qr" && (
                <div className="flex w-28 shrink-0 flex-col items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white p-2">
                  {isCoopReal ? (
                    coopPaymentQrImage ? (
                      <img src={coopPaymentQrImage} alt={t("Mã QR VNPAY")} className="h-20 w-20 object-contain" />
                    ) : (
                      <div className="flex h-20 w-20 items-center justify-center rounded bg-slate-50">
                        {coopPaymentQrBusy ? <Spinner /> : <span className="text-[10px] text-slate-400">VNPAY QR</span>}
                      </div>
                    )
                  ) : (
                  ...
                  <p className="w-full break-words text-center text-[10px] leading-tight text-slate-400">
                    {isCoopReal ? t("QR thanh toán VNPAY") : t("Nội dung: AFFREE {phone}", { phone: phone || "..." })}
                  </p>
                </div>
              )}
            </div>
          ...
          {phase === "form" && (
            <div className="space-y-3">
              ...
              <OrderInfoSection
                ...
                onName={setName}
                onPhone={setPhone}
                onAddress={isCoopReal ? updateCoopPrototypeAddress : setAddress}
              ...
              <div className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <div className="flex shrink-0 flex-col items-center gap-1 ml-auto">
                  ...
                  <div className="ml-auto shrink-0 rounded-lg bg-white px-3 py-1.5 text-xs font-semibold text-slate-500 ring-1 ring-slate-200">
                    {coopOrderLines.length} {t("món")}
                  </div>
                )}
              </div>
              {isMultiCoopOrder && (
                <div className="space-y-1.5 rounded-xl border border-slate-200 bg-white p-2.5">
                  {coopOrderLines.map((line) => (
                    <div key={`${line.offer.store.id}:${line.product.id}`} className="flex items-center justify-between gap-3 text-xs">
                      <span className="min-w-0 truncate text-slate-700">{line.product.name}</span>
                      <span className="shrink-0 font-semibold text-slate-900">
                        {formatMoney(line.offer.price, storeCurrency(activeOffer.store.id))} × {line.qty}
                      </span>
                    </div>
                  ))}
                </div>
              ...
              <Field label={cardEmailRequired ? t("Email (bắt buộc cho thanh toán thẻ)") : t("Email (tuỳ chọn)")}>
                <input
                  value={email}
                ...
                {cardEmailRequired && !email.trim() && (
                  <span className="mt-1 block text-xs text-slate-400">
                    {t("VNPay yêu cầu email khi nhập thẻ quốc tế.")}
                  </span>
                )}
              </Field>
                    ...
                    <div className="mt-2.5 space-y-2">
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-[11px] text-slate-400">
                          {cardCvvDigits.length >= 3
                            ...
                            : t("Nhập CVV để bật nút đặt hàng và auto-fill VNPay.")}
                        </p>
                        <button
                          type="button"
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder="CVV •••"
                          className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-1.5 text-sm font-mono outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100"
                        />
                      </div>
                    </div>
                  )}
          ...
          {phase === "running" && false && isCoopReal && (
            <div className="space-y-3">
              <ol className="space-y-2.5">
                ...
                <li className="flex items-start gap-2.5">
                  <span className="mt-0.5 shrink-0">
                    {coopBusy ? <Spinner /> : <CheckIcon />}
                  </span>
                  <p className="text-sm text-slate-500">
                    {coopBusy
                      ? t("Đang đăng nhập tài khoản đặt hộ, tạo giỏ và thêm sản phẩm Co.op…")
                      : t("Đã đăng nhập tài khoản đặt hộ, tạo giỏ và thêm sản phẩm Co.op.")}
                  </p>
                </li>
              </ol>
              {coopBrowserVisible && (
                <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
                  <div className="flex items-center justify-between gap-2 bg-slate-900 px-3 py-2 text-xs text-white">
                    <span>{coopBrowserStatus || t("Đang lấy mã QR VNPAY thật…")}</span>
                    <button type="button" onClick={closeCoopBrowser} className="rounded bg-white/10 px-2 py-1 font-semibold hover:bg-white/20">
                      {t("Đóng màn hình")}
                    </button>
                  </div>
                  <div className="flex min-h-72 items-center justify-center bg-white">
                    {coopBrowserFrame ? (
                      ...
                      <div className="flex items-center gap-2 py-12 text-sm text-slate-500">
                        <Spinner /> {t("Đang mở checkout và lấy mã QR VNPAY…")}
                      </div>
                    )}
                  </div>
                </div>
          ...
          {phase === "running" && !isBHXReal && (
            <div className={coopInlineBrowserPanel ? "grid gap-4 lg:grid-cols-[minmax(0,0.95fr)_minmax(520px,1.25fr)]" : "space-y-1"}>
              <div className="min-w-0">
                <ol className="space-y-2.5">
                        ...
                        {isCurrent && s.kind === "payment-select" && (
                          <PauseBox tone="blue" hint={demoPayMethod === "qr"
                            ...
                            <div className="space-y-2">
                              ...
                              {isCoopReal && demoPayMethod === "qr" && (
                                <button
                                  type="button"
                                  disabled={!coopPaymentQrImage || coopPaymentQrBusy || coopPaymentVerifying}
                                  onClick={() => {
                                    setCoopPaymentVerifying(true);
                                    sendCoopBrowserMessage({ type: "coop_payment_scanned" });
                                  }}
                                ...
                                >
                                  {coopPaymentVerifying ? t("Đang kiểm tra thanh toán…") : t("Đã quét QR →")}
                                </button>
                              )}
          ...
          {/* PHASE 3: thành công */}
          {phase === "done" && (
            <div className={coopInlineBrowserPanel ? "grid gap-4 py-4 lg:grid-cols-[minmax(0,0.9fr)_minmax(520px,1.25fr)]" : "flex flex-col items-center py-4 text-center"}>
              <div className={coopInlineBrowserPanel ? "flex min-w-0 flex-col items-center text-center" : "contents"}>
              ...
              {coopInlineBrowserPanel && (
                <div className="min-w-0 text-left lg:sticky lg:top-0 lg:self-start">
                  {coopInlineBrowserPanel}
                </div>
              )}
          ...
          {phase === "failed" && (
            <div className="flex flex-col items-center py-5 text-center">
              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-rose-100">
                <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="#e11d48" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M18 6 6 18" />
                  <path d="m6 6 12 12" />
                </svg>
              </div>
              <h3 className="mt-3 text-lg font-bold text-slate-900">
                {t("Thanh toán chưa thành công")}
              </h3>
              <p className="mt-1 max-w-sm text-sm text-slate-500">
                {coopError || submitError || t("Giao dịch đã thất bại hoặc bị hủy. Đơn chưa được ghi nhận là đặt hàng thành công.")}
              </p>
            ...
            phải cuộn xuống mới thấy, dễ tưởng popup bị cắt. */}
        {
          phase === "form" && (
            <div className="border-t border-slate-200 bg-white/95 px-4 py-3 backdrop-blur-sm">
              ...
              <button
                disabled={autoStart || !canStart || (isCoopReal || isBHXReal ? coopBusy : false)}
                onClick={() => {
                  flushProfile({ name, phone, address });
                  ...
                  if (demoPayMethod === "card" && !usingSavedCard && cardSaved && cardNum.replace(/\s/g, "").length >= 12 && /^\d{2}\/\d{2}$/.test(cardExp)) {
                    saveCard({ number: cardNum, name: name.trim().toUpperCase(), exp: cardExp, brand: null, cvv: cardCvv });
                  }
              ...
              >
                {isTXNNReal && submitting
                  ...
                  : autoStart && isCoopReal && !coopBusy
                    ? t("Đang tự kết nối Co.op…")
                  : coopBusy
              ...
              {!canStart && (
                <p className="mt-1.5 text-center text-xs text-slate-400">
                  {autoStart && isCoopReal
                    ? t("Đang chuẩn bị địa chỉ và cửa hàng Co.op gần nhất, bạn không cần bấm thêm.")
                    : isCoopReal
                    ? t("Nhập đủ tên, số điện thoại, địa chỉ và đảm bảo đơn Co.op từ 200.000đ.")
                    : cardEmailRequired && !emailValid && name.trim() && phoneValid && address.trim()
                      ? t("Nhập email hợp lệ để thanh toán thẻ qua VNPay.")
...
function prototypeSlotDate(label: string) {
  const date = new Date();
  const normalized = normalizeCoopText(label);
...
function isPrototypeCoopSlotPast(label: string) {
  const normalized = normalizeCoopText(label);
  if (normalized.includes("ngay mai") || /\bmai\b/.test(normalized)) return false;
  const range = prototypeSlotRange(label);
  const now = new Date();
  return now.getHours() * 60 + now.getMinutes() + COOP_DELIVERY_LEAD_MINUTES >= range.to;
}

function findClosestCoopSlot(
  slots: Array<{ from: string; to: string; disabled?: boolean }>,
  ...
  return slots
    ...
    .map((item) => {
      const from = parseSlotStartMinutes(item.from);
      const to = parseSlotStartMinutes(item.to);
...
function selectCoopPrototypePaymentMethods(methods: CoopPaymentMethod[]): CoopPaymentMethod[] {
  const byCode = (patterns: RegExp[]) => methods.find((method) => {
    const haystack = `${method.methodCode || ""} ${method.methodGroupCode || ""} ${method.name || ""}`;
    return !method.isDisabled && patterns.some((pattern) => pattern.test(haystack));
  });

  const qr = byCode([/VNPAY_GATEWAY_QR/i, /MOMO_GATEWAY/i, /\bQR\b/i]);
  const cod = byCode([/^COD\b/i, /thanh toán khi nhận/i, /tiền mặt/i]);
  const internationalCard = byCode([/INTERNATIONAL_CARD/i, /quốc tế/i, /VISA/i, /MASTERCARD/i]);

  const selected: CoopPaymentMethod[] = [];
  if (qr) selected.push({ ...qr, name: "QR" });
  if (cod) selected.push({ ...cod, name: "COD" });
  if (internationalCard) selected.push({ ...internationalCard, name: "Thẻ Quốc Tế" });
  return selected;
}
