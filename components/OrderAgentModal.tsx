...
import OrderInfoSection, { type DeliveryMode } from "./OrderInfoSection";
import { detectCardBrand, CARD_BRANDS, CARD_BRAND_STYLE } from "./PaymentSection";
import { ChainBadge } from "./ChainBadge";
import { MarqueeText } from "./MarqueeText";
import type { OrderProviderKey, OrderRequiredInput, PublicOrderSessionState } from "@/lib/order-agent/types";
import { startBHXOrder, submitBHXOtp, submitBHXFinalConfirm, type BhxOrderRuntime } from "@/lib/order-agent/bhx";
import { buildMissingAddressMessage, formatUsAddress, parseUsAddress } from "@/lib/order-agent/address";
...
export default function OrderAgentModal({
  ...
  /**
   * Nếu có: bấm nút xác nhận ở phase "form" sẽ GỌI onProceed (để mở màn chọn tài khoản
   * + đặt đơn co-browse 2 cột) thay vì chạy mô phỏng nội bộ. Form gốc giữ nguyên làm bước 1.
   */
  onProceed?: (payload: { offer: RankedOffer; qty: number; name: string; phone: string; address: string; slot: string; payMethod: "qr" | "card" | "cod" | null; cardLabel: string | null }) => void;
  lang?: Lang;
}) {
  const t = (vi: string, vars?: Record<string, string | number>) => tr(lang, vi, vars);
  const [phase, setPhase] = useState<"form" | "running" | "done">("form");
  // Đơn 1 chuỗi BHX (mua ngay 1 sản phẩm): chọn Giao tận nơi (mặc định) / Nhận tại cửa hàng.
  ...
  const orderSessionProvider = useMemo<OrderProviderKey | null>(() => {
    if (activeOffer.store.chain === "tuoixanhnhanhngon") return "tuoixanhnhanhngon";
    if (activeOffer.store.chain === "premiumoutlets") return "premiumoutlets";
    return null;
  }, [activeOffer.store.chain]);
  const usesOrderSessionAgent = Boolean(orderSessionProvider);
  ...
  const [interactionHoldState, setInteractionHoldState] = useState<"idle" | "holding" | "checking">("idle");
  const [interactionFrameRefreshKey, setInteractionFrameRefreshKey] = useState(0);
  const interactionPointerDownRef = useRef(false);
  const lastInteractionRatiosRef = useRef<{ xRatio: number; yRatio: number } | null>(null);
  const lastInteractionMoveAtRef = useRef(0);
  ...
  const savedCardHasFullNumber = (savedCard?.number || "").replace(/\D/g, "").length >= 12;
  const premiumOutletsRequiresCard = orderSessionProvider === "premiumoutlets";
  ...
  const premiumOutletsCardReady = demoPayMethod === "card" && (
    (usingSavedCard && savedCardHasFullNumber && /^\d{2}\/\d{2}$/.test(savedCard?.exp || "") && cardCvv.length >= 3)
    || newCardReady
  );
  ...
  useEffect(() => {
    ...
    const isRealtimeVerifyActive = Boolean(
      sessionId
      && serverState?.popupFrameAvailable
      && serverState?.popup?.kind === "human_verify"
      && serverState?.popup?.mode === "full-viewport"
      && serverState?.requiredInput === "captcha"
    );
    let alive = true;
    let inFlight = false;
    ...
    const nextDelay = (status?: PublicOrderSessionState["status"], options?: { idle?: number; errors?: number }) => {
      ...
      if (isRealtimeVerifyActive) {
        return 400;
      }

      if (status === "running" || status === "verifying_payment") {
        return Math.min(8000, 2500 + idle * 1500);
      }

      if (status?.startsWith("waiting_for_")) {
        return Math.min(6000, 2000 + idle * 1000);
      }

      return Math.min(10000, 4000 + idle * 2000);
    };
  ...
  }, [lang, phase, serverState?.popup?.kind, serverState?.popup?.mode, serverState?.popupFrameAvailable, serverState?.requiredInput, sessionId]);
  ...
  const popupKind = serverState?.popup?.kind || "";
  ...
  const popupIsTxnnPayment = popupKind === "payment";
  const popupIsHumanVerify = popupKind === "human_verify";
  const popupIsPaymentSelection = popupKind === "payment_selection";
  const popupIsFullscreenInteraction = serverState?.popup?.mode === "full-viewport";
  const popupSupportsHold = serverState?.popup?.interaction === "hold";
  const popupHoldDurationMs = serverState?.popup?.holdDurationMs ?? 1800;
  const popupVerifyState = popupIsHumanVerify ? serverState?.popup?.verifyState : undefined;
  const popupVerifyHint = popupIsHumanVerify ? serverState?.popup?.verifyHint : undefined;
  const popupHasRealtimeVerifyStream = Boolean(
    sessionId
    && serverState?.popupFrameAvailable
    && popupIsHumanVerify
    && popupIsFullscreenInteraction
    && serverState.requiredInput === "captcha"
  );
  const popupFrameTs = serverState?.popup?.updatedAt || serverState?.updatedAt;
  const popupFrameCacheKey = popupHasRealtimeVerifyStream
    ? interactionFrameRefreshKey || Date.now()
    : popupFrameTs;
  const popupFrameSrc = sessionId && popupFrameCacheKey
    ? `/api/order-sessions/${sessionId}/frame?ts=${encodeURIComponent(String(popupFrameCacheKey))}`
    : null;
  const premiumOutletsParsedAddress = useMemo(() => {
    if (!premiumOutletsRequiresCard) return null;
    return parseUsAddress(address);
  }, [address, premiumOutletsRequiresCard]);
  const premiumOutletsAddressError = useMemo(() => {
    if (!premiumOutletsRequiresCard || !address.trim()) return "";
    return premiumOutletsParsedAddress && !premiumOutletsParsedAddress.isComplete
      ? buildMissingAddressMessage(premiumOutletsParsedAddress.missingFields)
      : "";
  }, [address, premiumOutletsParsedAddress, premiumOutletsRequiresCard]);
  // Đơn 1 chuỗi BHX: nhận diện theo prefix "bhx" ở chain HOẶC store-id (client hay fallback
  // chain = raw id "bhx-q1"/"BHX_00003" vì không resolve được store). Nhận tại CH → không cần địa chỉ.
  const isBHX = /^bhx/i.test(activeOffer.store?.chain || "") || /^bhx/i.test(activeOffer.store?.id || "");
  const isPickup = isBHX && deliveryMode === "pickup";
  // proceedMode: form này chỉ là BƯỚC 1 (chuyển sang chọn tài khoản + màn 2 cột co-browse).
  // User CHỌN thanh toán ở đây → qua trang cửa hàng, bot tự chọn đúng phương thức này.
  const proceedMode = Boolean(onProceed);
  const canStart = proceedMode
    ? (!!name.trim() && phoneValid && (isPickup || !!address.trim()) && qty >= minQty && (!payTabsShown || demoPayMethod !== null))
    : (!!name.trim() &&
      phoneValid &&
      ...
      (!premiumOutletsRequiresCard || Boolean(premiumOutletsParsedAddress?.isComplete)) &&
      (!premiumOutletsRequiresCard || (demoPayMethod === "card" && premiumOutletsCardReady)) &&
      (!isCoopReal || coopCanStart));

  const premiumOutletsVerifyBanner = useMemo(() => {
    if (!popupIsFullscreenInteraction) return null;
    if (popupVerifyState === "passed") {
      return {
        className: "rounded-lg border border-emerald-300 bg-emerald-50 px-2.5 py-2 text-[11px] font-semibold text-emerald-900",
        text: popupVerifyHint || t("Widget đã pass, thả chuột ngay."),
      };
    }
    if (popupVerifyState === "checking") {
      return {
        className: "rounded-lg border border-sky-200 bg-sky-50 px-2.5 py-2 text-[11px] font-medium text-sky-900",
        text: popupVerifyHint || t("Đã thả chuột. Worker đang kiểm tra verify đã biến mất chưa để tự tiếp tục mua hàng."),
      };
    }
    if (popupVerifyState === "failed") {
      return {
        className: "rounded-lg border border-rose-200 bg-rose-50 px-2.5 py-2 text-[11px] font-medium text-rose-900",
        text: popupVerifyHint || t("Verify chưa pass / đã reset, vui lòng giữ lại hoặc thử lại."),
      };
    }
    if (popupVerifyState === "holding") {
      return {
        className: "rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 py-2 text-[11px] font-medium text-emerald-900",
        text: popupVerifyHint || t("Đang giữ verify… Worker sẽ báo ngay khi widget pass để bạn thả chuột."),
      };
    }
    if (popupVerifyHint) {
      return {
        className: "rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-2 text-[11px] text-slate-700",
        text: popupVerifyHint,
      };
    }
    if (interactionHoldState === "holding") {
      return {
        className: "rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 py-2 text-[11px] font-medium text-emerald-900",
        text: t("Đang giữ verify… Khi widget hiện trạng thái pass (tick xanh / biến mất), bạn thả chuột ngay."),
      };
    }
    if (interactionHoldState === "checking") {
      return {
        className: "rounded-lg border border-sky-200 bg-sky-50 px-2.5 py-2 text-[11px] font-medium text-sky-900",
        text: t("Đã thả chuột. Worker đang kiểm tra verify đã biến mất chưa để tự tiếp tục mua hàng."),
      };
    }
    return null;
  }, [interactionHoldState, popupIsFullscreenInteraction, popupVerifyHint, popupVerifyState, t]);

  useEffect(() => {
    if (!popupIsHumanVerify || !popupIsFullscreenInteraction || serverState?.requiredInput !== "captcha") {
      interactionPointerDownRef.current = false;
      lastInteractionRatiosRef.current = null;
      setInteractionHoldState("idle");
    }
  }, [popupIsFullscreenInteraction, popupIsHumanVerify, serverState?.requiredInput]);

  useEffect(() => {
    if (!popupHasRealtimeVerifyStream) {
      setInteractionFrameRefreshKey(0);
      return;
    }
    setInteractionFrameRefreshKey(Date.now());
    const timer = window.setInterval(() => {
      setInteractionFrameRefreshKey(Date.now());
    }, 180);
    return () => window.clearInterval(timer);
  }, [popupHasRealtimeVerifyStream]);
  ...
  function handlePopupFrameClick(event: React.MouseEvent<HTMLImageElement>) {
    ...
    if (popupSupportsHold) {
      void sendSessionEvent({ type: "popup_hold", xRatio, yRatio, durationMs: popupHoldDurationMs });
      return;
    }
    void sendSessionEvent({ type: "popup_click", xRatio, yRatio });
  }

  function getInteractionFrameRatios(event: React.MouseEvent<HTMLImageElement>) {
    const rect = event.currentTarget.getBoundingClientRect();
  ...
  function releaseInteractionFrame(ratios: { xRatio: number; yRatio: number } | null | undefined) {
    if (!popupIsFullscreenInteraction || !interactionPointerDownRef.current || !ratios) return;
    interactionPointerDownRef.current = false;
    lastInteractionRatiosRef.current = ratios;
    setInteractionHoldState("checking");
    void sendSessionEvent({ type: "frame_mouseup", ...ratios });
  }

  function handleInteractionFrameMouseDown(event: React.MouseEvent<HTMLImageElement>) {
    if (!popupIsFullscreenInteraction) return;
    const ratios = getInteractionFrameRatios(event);
    if (!ratios) return;
    interactionPointerDownRef.current = true;
    lastInteractionRatiosRef.current = ratios;
    setInteractionHoldState("holding");
    void sendSessionEvent({ type: "frame_mousedown", ...ratios });
  }

  function handleInteractionFrameMouseMove(event: React.MouseEvent<HTMLImageElement>) {
    if (!popupIsFullscreenInteraction || !interactionPointerDownRef.current) return;
    const now = Date.now();
    if (now - lastInteractionMoveAtRef.current < 50) return;
    lastInteractionMoveAtRef.current = now;
    const ratios = getInteractionFrameRatios(event);
    if (!ratios) return;
    lastInteractionRatiosRef.current = ratios;
    void sendSessionEvent({ type: "frame_mousemove", ...ratios });
  }

  function handleInteractionFrameMouseUp(event: React.MouseEvent<HTMLImageElement>) {
    releaseInteractionFrame(getInteractionFrameRatios(event) || lastInteractionRatiosRef.current);
  }

  function handleInteractionFrameMouseLeave(event: React.MouseEvent<HTMLImageElement>) {
    releaseInteractionFrame(getInteractionFrameRatios(event) || lastInteractionRatiosRef.current);
  }

  function preventFrameDrag(event: React.DragEvent<HTMLImageElement>) {
  ...
  async function createSession() {
    const premiumOutletsAddress = premiumOutletsRequiresCard ? parseUsAddress(address) : null;
    if (premiumOutletsRequiresCard && (!premiumOutletsAddress || !premiumOutletsAddress.isComplete)) {
      setSubmitError(buildMissingAddressMessage(premiumOutletsAddress?.missingFields || ["street", "city", "state", "zipCode"]));
      return;
    }

    const normalizedAddress = premiumOutletsAddress ? formatUsAddress(premiumOutletsAddress) : address.trim();
    flushProfile({ name, email, phone, address: normalizedAddress });
    ...
    const premiumOutletsPayment = premiumOutletsRequiresCard
      ? {
          method: "card" as const,
          card: {
            number: (usingSavedCard && savedCard ? savedCard.number : cardNum).replace(/\D/g, ""),
            expMonth: expMonthRaw.replace(/\D/g, "").slice(0, 2),
            expYear: expYearRaw.replace(/\D/g, "").slice(-2),
            cvv: cardCvv.replace(/\D/g, "").slice(0, 4),
            name: (usingSavedCard && savedCard?.name ? savedCard.name : name).trim() || undefined,
          },
        }
      : undefined;
      ...
      const res = await fetch("/api/order-sessions", {
        ...
        body: JSON.stringify({
          ...
          customer: {
            ...
            usAddress: premiumOutletsAddress
              ? {
                  street: premiumOutletsAddress.street,
                  city: premiumOutletsAddress.city,
                  state: premiumOutletsAddress.state,
                  zipCode: premiumOutletsAddress.zipCode,
                }
              : undefined,
          },
          delivery: cfg.needSlot ? { slot } : undefined,
          payment: premiumOutletsPayment,
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 py-4">
          ...
          {/* Banner: bản mô phỏng */}
          <div
            className={`mb-4 rounded-lg border px-3 py-2 text-xs ${!isTXNNReal && !isCoopReal ? "text-center " : ""}${isTXNNReal
          ...
          {phase === "form" && (
            <div className="space-y-3">
              ...
              <OrderInfoSection
                ...
                addressError={premiumOutletsAddressError || undefined}
                ...
                pickup={
                  isBHX
                    ? {
                        ...
                        storeLabel: `${activeOffer.store.name}${activeOffer.store.address ? ` (${activeOffer.store.address})` : ""}`,
                      }
                    : undefined
                }
              />
              ...
              {/* Khung giờ giao — chỉ hiện khi cần; ẩn khi chọn "Nhận tại cửa hàng" (pickup) */}
              {cfg.needSlot && !isPickup && (
              ...
              {/* ── Thanh toán — tabs ngang CÙNG CẤU TRÚC với form giỏ hàng ──
                  proceedMode: user chọn ở ĐÂY, qua trang cửa hàng bot tự chọn đúng phương thức này. */}
                ...
                <section className="rounded-2xl border border-slate-400 p-3">
                  ...
                  {demoPayMethod === "card" && usingSavedCard && savedCard && (
                    <div className="mt-2.5 space-y-2">
                      ...
                      {premiumOutletsRequiresCard && (
                        <div className="space-y-2 rounded-lg border border-emerald-200 bg-white px-3 py-2 text-left">
                          <p className="text-[11px] font-medium text-slate-600">
                            {savedCardHasFullNumber
                              ? t("Premium Outlets live cần CVV hiện tại để worker tự điền card form; CVV không được lưu trên máy.")
                              : t("Thẻ từ tài khoản chỉ còn 4 số cuối nên không đủ cho Premium Outlets live autofill. Hãy chuyển sang thẻ local đầy đủ hoặc nhập thẻ mới.")}
                          </p>
                          {savedCardHasFullNumber ? (
                            <input
                              type="password"
                    ...
                    <li key={i} className="flex items-start gap-2.5">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {isCurrent && s.kind === "captcha" && (
                          <PauseBox tone="amber" hint={popupIsHumanVerify
                            ? t("🛡️ Website đang yêu cầu human verification. Ảnh bên dưới là live fullscreen interaction frame từ worker để bạn thao tác trực tiếp.")
                            : t("🤖 Trợ lý không vượt CAPTCHA. Bạn xác minh giúp (mô phỏng).")}>
                            {usesServerTimeline && popupIsHumanVerify && serverState?.popupFrameAvailable && serverState.popup ? (
                              <div className="space-y-3">
                                <div className="rounded-lg border border-amber-200 bg-white p-3">
                                  <div className="mb-2 flex items-start justify-between gap-3">
                                    <div>
                                      <p className="text-sm font-semibold text-slate-800">
                                        {serverState.popup.title || t("Human verification")}
                                      </p>
                                      <p className="mt-1 text-xs text-slate-500">
                                        {popupIsFullscreenInteraction
                                          ? t("Nhấn và giữ trực tiếp trên live frame bên dưới. Đây là toàn viewport worker, không còn auto-hold bot cho Premium Outlets.")
                                          : popupSupportsHold
                                            ? t("Click trực tiếp vào ảnh để worker nhấn giữ đúng vị trí verify.")
                                            : t("Click trực tiếp vào ảnh để thao tác đúng vùng verify trên website thật.")}
                                      </p>
                                    </div>
                                    <span className="rounded-full bg-amber-100 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-amber-700">
                                      {popupIsFullscreenInteraction ? t("full-viewport") : popupSupportsHold ? t("hold") : t("click")}
                                    </span>
                                  </div>
                                  <img
                                    src={popupFrameSrc || undefined}
                                    alt={serverState.popup.title || t("Human verification")}
                                    className={`mx-auto rounded-lg border border-amber-200 bg-white ${popupIsFullscreenInteraction ? "max-h-[34rem] w-full cursor-grab object-contain active:cursor-grabbing" : "max-h-[26rem] w-auto cursor-crosshair"}`}
                                    draggable={false}
                                    onClick={popupIsFullscreenInteraction ? undefined : handlePopupFrameClick}
                                    onMouseDown={popupIsFullscreenInteraction ? handleInteractionFrameMouseDown : undefined}
                                    onMouseMove={popupIsFullscreenInteraction ? handleInteractionFrameMouseMove : undefined}
                                    onMouseUp={popupIsFullscreenInteraction ? handleInteractionFrameMouseUp : undefined}
                                    onMouseLeave={popupIsFullscreenInteraction ? handleInteractionFrameMouseLeave : undefined}
                                    onDragStart={preventFrameDrag}
                                  />
                                  <div className="mt-2 space-y-1">
                                    ...
                                    {serverState.popup.actions.length > 0 && (
                                      <p className="text-[11px] text-slate-500">
                                        {t("Vùng / nút detect được")}: {serverState.popup.actions.join(" · ")}
                                      </p>
                                    )}
                                    {premiumOutletsVerifyBanner && (
                                      <p className={premiumOutletsVerifyBanner.className}>
                                        {premiumOutletsVerifyBanner.text}
                                      </p>
                                    )}
                                    {popupIsFullscreenInteraction && (
                                      <p className="rounded-lg bg-slate-50 px-2.5 py-2 text-[11px] text-slate-600">
                                        {t("Giữ chuột trên đúng nút Press & Hold. Khi worker báo widget đã pass thì thả chuột; worker sẽ tự dò verify biến mất và tiếp tục lại luồng Premium Outlets.")}
                                      </p>
                                    )}
                                  </div>
                                </div>
                                ...
                                <div className="grid gap-2 sm:grid-cols-2">
                                  <button
                                    ...
                                    className="rounded-lg border border-amber-300 bg-white px-4 py-2.5 text-sm font-semibold text-amber-900 transition hover:bg-amber-50"
                                  >
                                    {popupIsFullscreenInteraction ? t("Mở live frame lớn") : t("Mở vùng verify lớn")}
                                  </button>
                                  <a
                                ...
                                <button
                                  ...
                                  className="flex w-full items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50"
                                >
                                  {t("Tôi đã xử lý xong verify, tiếp tục")}
                                </button>
                              </div>
                            ) : (
                              <button
                        ...
                        {isCurrent && s.kind === "payment-select" && (
                          <PauseBox tone="blue" hint={usesServerTimeline && popupIsPaymentSelection
                            ? t("💳 Worker đã điền xong checkout và đang cast đúng vùng payment method trên website thật để bạn chọn.")
                            : demoPayMethod === "qr"
                            ...
                            {usesServerTimeline && popupIsPaymentSelection && serverState?.popupFrameAvailable && serverState.popup ? (
                              <div className="space-y-3">
                                <div className="rounded-lg border border-blue-200 bg-white p-3">
                                  <div className="mb-2 flex items-start justify-between gap-3">
                                    <div>
                                      <p className="text-sm font-semibold text-slate-800">
                                        {serverState.popup.title || t("Payment methods")}
                                      </p>
                                      <p className="mt-1 text-xs text-slate-500">
                                        {t("Click trực tiếp vào ảnh để chọn payment method / payment tab đúng trên website thật. Sau mỗi lần click, worker sẽ đọc lại trạng thái checkout.")}
                                      </p>
                                    </div>
                                  ...
                                  <img
                                    src={popupFrameSrc || undefined}
                                    alt={serverState.popup.title || t("Payment methods")}
                                    className="mx-auto max-h-[26rem] w-auto rounded-lg border border-blue-200 bg-white cursor-crosshair"
                                  ...
                                  <div className="mt-2 space-y-1">
                                    ...
                                    {serverState.popup.actions.length > 0 && (
                                      <p className="text-[11px] text-slate-500">
                                        {t("Phương thức detect được")}: {serverState.popup.actions.join(" · ")}
                                      </p>
                                    )}
                                  </div>
                                </div>
                                ...
                                <div className="grid gap-2 sm:grid-cols-2">
                                  <button
                                    type="button"
                                    onClick={() => setPopupViewerOpen(true)}
                                    className="rounded-lg border border-blue-300 bg-white px-4 py-2.5 text-sm font-semibold text-blue-900 transition hover:bg-blue-50"
                                  >
                                    {t("Mở vùng payment lớn")}
                                  </button>
                                  <a
                                    href={serverState?.handoffUrl || activeOffer.productUrl}
                                    target="_blank"
                                  ...
                                  <div className="space-y-2 rounded-xl border border-slate-200 bg-white p-3">
                                    ...
                                    <div>
                                      ...
                                      <input
                                        ...
                                        className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 font-mono text-sm outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400"
                                      />
                                    </div>
                                    <div className="flex gap-2">
          ...
          phase === "form" && (
            <div className="border-t border-slate-200 bg-white/95 px-4 py-3 backdrop-blur-sm">
              ...
              <button
                ...
                onClick={() => {
                  flushProfile({ name, phone, address });
                  // Có onProceed → form gốc là BƯỚC 1; chuyển sang chọn tài khoản + màn 2 cột co-browse.
                  if (onProceed) {
                    onProceed({
                      offer: activeOffer, qty, name, phone, address, slot, payMethod: demoPayMethod,
                      // Thẻ nhập/lưu ở Affree (che số) — trợ lý dùng đưa qua trang cửa hàng.
                      cardLabel: demoPayMethod === "card" && cardLast4 ? maskedCardLabel : null,
                    });
                    return;
                  }
                  ...
                  if (demoPayMethod === "card" && !usingSavedCard && cardSaved && cardNum.replace(/\s/g, "").length >= 12 && /^\d{2}\/\d{2}$/.test(cardExp)) {
                    saveCard({ number: cardNum, name: cardName.trim() || name.trim().toUpperCase(), exp: cardExp, brand: detectCardBrand(cardNum), cvv: cardCvv });
                  }
                  // Snapshot: thẻ đã đủ ngay ở form → bước thẻ tự chạy, không hỏi nhập lại.
                  setCardConfirmed(demoPayMethod === "card" && cardReady);
                  if (usesOrderSessionAgent) {
                    void createSession();
                  } else if (isCoopReal) {
                    void startCoopOrder();
                  } else if (isBHXReal) {
              ...
              >
                {usesOrderSessionAgent && submitting
              ...
              {!canStart && (
                <p className="mt-1.5 text-center text-xs text-slate-400">
                  {proceedMode
                  ? (payTabsShown && demoPayMethod === null && name.trim() && phoneValid && (isPickup || address.trim())
                    ? t("Chọn phương thức thanh toán để tiếp tục.")
                    : isPickup ? t("Nhập đủ tên, số điện thoại để tiếp tục.") : t("Nhập đủ tên, số điện thoại và địa chỉ để tiếp tục."))
                  : isCoopReal
                  ? t("Nhập đủ tên, số điện thoại, địa chỉ và đảm bảo đơn Co.op từ 200.000đ.")
                  : premiumOutletsRequiresCard && name.trim() && phoneValid && address.trim() && !!premiumOutletsAddressError
                    ? premiumOutletsAddressError
                    : premiumOutletsRequiresCard && name.trim() && phoneValid && address.trim() && emailValid && demoPayMethod === "card" && !premiumOutletsCardReady
                    ? t("Premium Outlets live cần thẻ đầy đủ có thể autofill; nếu dùng thẻ đã lưu thì nhập thêm CVV hiện tại.")
                    : premiumOutletsRequiresCard && payTabsShown && demoPayMethod !== "card" && name.trim() && phoneValid && address.trim()
                      ? t("Premium Outlets live hiện chỉ hỗ trợ card-first autofill. Chọn phương thức Thẻ để tiếp tục.")
                      : payTabsShown && demoPayMethod === null && name.trim() && phoneValid && address.trim()
                        ? t("Chọn phương thức thanh toán để tiếp tục.")
                        : isPickup
                          ? t("Nhập đủ tên, số điện thoại để bắt đầu.")
            ...
            <div className="flex items-start justify-between gap-3 border-b border-white/10 px-4 py-3 text-white">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs text-white/70">
                  {popupIsFullscreenInteraction
                    ? t("Ảnh bên dưới là live fullscreen interaction frame. Nhấn và giữ trực tiếp để thao tác trên website thật.")
              ...
              <div className="flex shrink-0 items-center gap-2">
                <span className="rounded-full bg-emerald-500/15 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-emerald-300">
                  {serverState.popup.mode || "popup-focus"}
                </span>
                <button
                  type="button"
                  onClick={() => setPopupViewerOpen(false)}
              ...
              <div className="flex min-h-full flex-col gap-4 lg:flex-row">
                <div className="flex min-h-[320px] flex-1 items-center justify-center rounded-2xl border border-white/10 bg-black/20 p-2 sm:p-4">
                  <img
                    src={popupFrameSrc}
                    alt={serverState.popup.title || t("Popup thanh toán")}
                    className={`max-h-[78vh] max-w-full rounded-xl border border-slate-700 bg-white object-contain ${popupIsFullscreenInteraction ? "w-full cursor-grab active:cursor-grabbing" : "w-auto cursor-crosshair"}`}
                    draggable={false}
                    onClick={popupIsFullscreenInteraction ? undefined : handlePopupFrameClick}
                    onMouseDown={popupIsFullscreenInteraction ? handleInteractionFrameMouseDown : undefined}
                    onMouseMove={popupIsFullscreenInteraction ? handleInteractionFrameMouseMove : undefined}
                ...
                <div className="w-full shrink-0 space-y-3 lg:w-80">
                  ...
                  <div className={popupVerifyState === "failed"
                    ? "rounded-xl border border-rose-500/30 bg-rose-500/10 px-3 py-2 text-xs text-rose-100"
                    : popupVerifyState === "checking"
                      ? "rounded-xl border border-sky-500/30 bg-sky-500/10 px-3 py-2 text-xs text-sky-100"
                      : popupVerifyState === "passed"
                        ? "rounded-xl border border-emerald-400/40 bg-emerald-500/15 px-3 py-2 text-xs text-emerald-100"
                        : "rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-100"}>
                    {popupIsFullscreenInteraction
                      ? premiumOutletsVerifyBanner?.text || t("Mẹo: đây là full viewport worker. Giữ chuột ngay trên widget verify; khi widget pass thì thả chuột để worker tiếp tục.")
                      : t("Mẹo: popup này đã được phóng to, nhưng click vẫn map theo đúng tỷ lệ về popup thật trên website nguồn.")}
                  </div>

                  <div className="space-y-2">
                    {popupIsTxnnPayment && !popupShowingQr && (
                      <button
                        type="button"
                        onClick={() => void switchPopupView("qr")}
