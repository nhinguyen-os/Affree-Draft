...
export default function OrderAgentModal({
  ...
  defaultQty?: number;
  onClose: () => void;
  onPlaced: (orderCode: string, chosen: RankedOffer, note?: string, pay?: DemoPayMethod | null) => void;
  /**
  ...
  useEffect(() => {
    ...
    if (serverState.status === "completed" && serverState.orderCode && !placedRef.current) {
      ...
      setPhase("done");
      onPlaced(serverState.orderCode, activeOffer, note, demoPayMethod);
      return;
    }
  ...
  useEffect(() => {
    ...
    if (current.kind === "success") {
      const timer = window.setTimeout(() => {
        ...
        setPhase("done");
        onPlaced("DEMO", activeOffer, note, demoPayMethod);
      }, 800);
  ...
  const popupFrameSrc = sessionId && popupFrameTs
    ? `/api/order-sessions/${sessionId}/frame?ts=${encodeURIComponent(popupFrameTs)}`
    : null;
  ...
  const placeCoopOrder = async () => {
    ...
    try {
      ...
      setCoopStep("success");
      setPhase("done");
      onPlaced(code, activeOffer, note, demoPayMethod);
    } catch (err) {
      setCoopError(err instanceof Error ? err.message : String(err));
    } finally {
      setCoopBusy(false);
  ...
  return (
    ...
    >
      ...
      >
              ...
              <div className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <div className="min-w-[9rem] flex-1">
                  ...
                  <button
                    ...
                    onClick={() => onOpenStore?.(activeOffer.store)}
        ...
        {/* Footer sticky — Tạm tính + nút "Đặt hàng ngay" luôn hiển thị (kể cả khi
