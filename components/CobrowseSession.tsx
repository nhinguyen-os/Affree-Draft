...
export default function CobrowseSession({
  ...
  function confirmConsent() {
    // KHÔNG tự mở cửa hàng đầu — hiện ghi chú "Chưa có phiên đặt hàng nào" ở khung phải
    // (theo prototype), user bấm tab cửa hàng để mở phiên.
    setStep("session");
  }
  ...
  const Header = (
    <>
      ...
      <div className="disclaimer-strip">
        <span className="disclaimer-text"><b>{t("Miễn trừ:")}</b> {t("Affree không liên kết, không đại diện cho các nhà bán được hiển thị (Bách Hóa Xanh, Co.op Online…). Đơn hàng được xác lập trực tiếp giữa bạn và nhà bán trên website chính chủ của họ. Affree không nhìn thấy và không lưu thông tin thanh toán của bạn.")}</span>
...
.cbz-x{background:none;border:none;color:#C8D8D3;font-size:24px;line-height:1;padding:0 4px;cursor:pointer;flex:none}
.cbz-x:hover{color:#fff}
.cbz .disclaimer-strip{background:var(--ink);color:#C8D8D3;font-size:11.5px;padding:8px 18px;line-height:1.5;display:flex;align-items:center;gap:14px}
