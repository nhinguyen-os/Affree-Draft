...
export default function TuiAgentModal({
  ...
  /** Click tên nguồn → mở trang bán hàng của nguồn đó (cha đóng modal + mở trang theo chuỗi). */
  onOpenStore?: (chain: string, label: string) => void;
}) {
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 py-4">
          ...
          {phase === "form" && (
            <div className="space-y-3">
              ...
              {groups.map((g) => (
                <section key={g.source} className="rounded-2xl border border-slate-400 p-3">
                  <div className="mb-2.5 flex items-center gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      {/* Click tên nguồn → mở trang bán hàng của nguồn đó */}
                      <button
                        type="button"
                        disabled={!onOpenStore || !g.chain}
                        onClick={() => g.chain && onOpenStore?.(g.chain, g.source)}
