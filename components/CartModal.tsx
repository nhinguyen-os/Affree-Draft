...
import { coopOrderExecutor } from "@/lib/order-executors/coop";
import type { StoreOrderContext, StoreOrderGroup, StoreOrderProgress } from "@/lib/order-executors/types";
...
async function getAgentWsUrl(sessionId: string, chain?: string) {
  ...
  try {
    ...
    /* agent-server token is optional in local dev */
  }
...
function parseCoopAddressParts(fullAddress: string): CoopAddressParts {
  ...
  const provinceCode = normalizeCoopText(provinceName).includes("ho chi minh") ? HCM_PROVINCE.code : "";
...
function toVnpayAscii(value: string) {
  return value
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/đ/g, "d")
...
export default function CartModal({
  ...
  const [orderProgress, setOrderProgress] = useState<Record<string, StoreOrderProgress>>({});
  ...
  const realOrderRunRef = useRef(false);
  const paymentWsRef = useRef<Map<string, WebSocket>>(new Map());
  const paymentPointerDownRef = useRef(false);
  const lastPaymentMoveAtRef = useRef(0);
  const nextRealOrderIndexRef = useRef(0);
  const groupAccountSlotsRef = useRef<Record<string, number>>({});
  const [coopAccountSlots, setCoopAccountSlots] = useState(1);
  const [paymentStreams, setPaymentStreams] = useState<Record<string, PaymentStreamState>>({});

  useEffect(() => {
    let alive = true;
    void fetch("/api/coop/order", {
      method: "POST",
      ...
      .then((data: { active?: number }) => {
        if (alive && Number.isFinite(data.active)) setCoopAccountSlots(Math.max(1, Number(data.active)));
      })
      .catch(() => {
        if (alive) setCoopAccountSlots(1);
      });
    return () => { alive = false; };
  }, []);
  ...
  const storeGroups = useMemo((): StoreGroup[] => {
    ...
    for (const item of items) {
      const store = item.offer.store;
      const terminalCode = store.terminalCode?.trim();
      const key = store.chain === "coop" && terminalCode ? `coop:${terminalCode}` : store.id;
      if (!map.has(key)) map.set(key, []);
      map.get(key)!.push(item);
    }
    const groups = Array.from(map.entries()).map(([key, storeItems]) => {
      const store = storeItems[0].offer.store;
      const cfg = getOrderConfig(store.chain);
      const originalStoreIds = Array.from(new Set(storeItems.map((item) => item.offer.store.id)));
      return {
        key,
        storeId: store.id,
        originalStoreIds,
        terminalCode: store.terminalCode?.trim() || undefined,
  ...
  const orderExecutors = [coopOrderExecutor];
  const hasRealExecutor = (group: StoreGroup) => {
    const orderGroup = toOrderGroup(group);
    return orderExecutors.some((executor) => executor.supports(orderGroup));
  };
  const missingTerminalGroups = storeGroups.filter((group) => group.chain === "coop" && !group.terminalCode);
  const missingTerminalError = missingTerminalGroups.length
    ? t("Có cửa hàng Co.opmart chưa có terminal_code nên chưa thể tạo đơn Co.op chính xác.")
    : "";

  function toOrderGroup(group: StoreGroup): StoreOrderGroup {
    return {
      key: group.key,
      chain: group.chain,
      storeId: group.storeId,
      terminalCode: group.terminalCode,
      items: group.items,
    };
  }
  ...
  const hasCoopGroups = storeGroups.some((group) => group.chain === "coop");
  const coopAddressParts = useMemo(() => parseCoopAddressParts(address), [address]);
  const coopFullAddress = [coopAddressParts.addressLine, coopAddressParts.wardName, coopAddressParts.districtName, coopAddressParts.provinceName]
    .map((item) => item.trim())
    .filter(Boolean)
    .join(", ");

  const coopOrderContext = (group: StoreGroup, accountSlot: number): StoreOrderContext => ({
    name,
    phone,
    address: coopFullAddress || address,
    email: storeExtras[group.key]?.email?.trim() || coopEmail.trim() || undefined,
  ...
  // Thẻ hợp lệ để auto-fill cổng thật: cần full số + CVV. Thẻ lấy từ account
  // chỉ có last4 thì không đủ payload, user phải dùng thẻ khác/full số.
  const savedCardReady =
    usingSavedCard &&
    activeCardNumber.replace(/\D/g, "").length >= 12 &&
    /^\d{2}\/\d{2}$/.test(savedCard?.exp || "") &&
    (savedCard?.cvv || cardCvv).replace(/\D/g, "").length >= 3;
  const newCardReady =
    cardNumber.replace(/\s/g, "").length >= 12 &&
    cardName.trim().length > 0 &&
    /^\d{2}\/\d{2}$/.test(cardExp) &&
    cardCvv.length >= 3;
  const cardReady = savedCardReady || newCardReady;
  ...
  const coopPaymentCardPayload = () => {
    if (payMethod !== "card" || !cardReady) return undefined;
    const number = activeCardNumber.replace(/\D/g, "");
    const [expiryMonth = "", expiryYearRaw = ""] = (usingSavedCard ? savedCard?.exp : cardExp)?.split("/") ?? [];
    const expiryYearDigits = expiryYearRaw.replace(/\D/g, "");
    const cvv = (usingSavedCard ? savedCard?.cvv || cardCvv : cardCvv).replace(/\D/g, "");
    if (number.length < 12 || !expiryMonth || expiryYearDigits.length < 2 || cvv.length < 3) return undefined;
    return {
      cardNumber: number,
      cardholderName: (usingSavedCard ? savedCard?.name : cardName)?.trim() || name,
      expiryMonth: expiryMonth.replace(/\D/g, "").padStart(2, "0").slice(0, 2),
      expiryYear: expiryYearDigits.length === 2 ? `20${expiryYearDigits}` : expiryYearDigits.slice(0, 4),
      cvv,
      email: coopEmail.trim(),
      billingAddress: toVnpayAscii(coopFullAddress || address),
      addressLine: toVnpayAscii(coopAddressParts.addressLine || address),
      city: toVnpayAscii(coopAddressParts.provinceName || "Ho Chi Minh"),
      district: toVnpayAscii(coopAddressParts.districtName),
      ward: toVnpayAscii(coopAddressParts.wardName),
      brand: cardBrand,
    };
  };
  ...
  const paymentReady = payMethod !== null && (payMethod !== "card" || cardReady);
  const orderHint =
  ...
  const agentPlan = useMemo(() => storeGroups.map((g) => {
    ...
    if (cfg.needSlot) steps.push({ label: t('Chọn khung giờ "{slot}"…', { slot: t(slotOf(g.key)) }) });
    steps.push({ label: t("Xác nhận & gửi đơn tới {chain}…", { chain: g.storeName }) });
    // Co.op/VNPAY chỉ có QR thật sau khi xác nhận đặt hàng và có paymentUrl.
    if (payMethod === "qr") steps.push({ label: t("Lấy mã QR thanh toán thật từ VNPAY…"), pause: "pay-qr" });
    else if (payMethod === "card") steps.push({ label: t("Mở cổng thanh toán thẻ…"), pause: "pay-card" });
  ...
  async function finalizeOrder() {
      ...
      try {
        for (const item of group.items) {
          await addPurchase({
            ...
            storeId: item.offer.store.id,
            storeName: item.offer.store.name || group.storeName,
            chain: group.chain,
            qty: item.qty,
            ...
            buyerPhone: phone,
            buyerAddr: address,
            buyerNote: [group.needSlot ? slotOf(group.key) : "", storeExtras[group.key]?.email || "", storeExtras[group.key]?.note?.trim() || ""]
              .filter(Boolean)
              .join(" | "),
          });
        }
    ...
    setResults(next);
    const realCodes = storeGroups
      .map((group) => orderProgress[group.key]?.orderCode)
      .filter(Boolean);
    setOrderCode(realCodes.length ? realCodes.join(" · ") : "AFF-" + String(Date.now()).slice(-6) + "-" + Math.floor(Math.random() * 900 + 100));
    setStoreCodes(codes);
    setPhase("done");
    const okStoreIds = storeGroups
      .filter((group) => next[group.key] === "ok")
      .flatMap((group) => group.originalStoreIds);
  ...
  function progressStepIndex(group: StoreGroup, phaseName: StoreOrderProgress["phase"]) {
    const steps = agentPlan.find((plan) => plan.group.key === group.key)?.steps ?? [];
    if (!steps.length) return 0;
    if (phaseName === "completed") return steps.length - 1;
    if (phaseName === "preparing") return Math.min(2, steps.length - 1);
    if (phaseName === "ready_to_confirm" || phaseName === "placing_order") {
      const confirmIndex = steps.findIndex((step) => step.label.includes(t("Xác nhận")));
      return confirmIndex >= 0 ? confirmIndex : Math.max(steps.length - 2, 0);
    }
    if (phaseName === "fetching_payment_qr") {
      const payIndex = steps.findIndex((step) => Boolean(step.pause));
      return payIndex >= 0 ? payIndex : Math.max(steps.length - 2, 0);
    }
    if (phaseName === "awaiting_payment") {
      const payIndex = steps.findIndex((step) => Boolean(step.pause));
      return payIndex >= 0 ? payIndex : Math.max(steps.length - 2, 0);
    }
    return phaseName === "creating_cart" ? Math.min(1, steps.length - 1) : 0;
  }

  async function runRealOrders() {
    if (realOrderRunRef.current) return;
    realOrderRunRef.current = true;
    nextRealOrderIndexRef.current = 0;
    groupAccountSlotsRef.current = {};
    const realGroupCount = storeGroups.filter((group) => hasRealExecutor(group) || (group.chain === "coop" && !group.terminalCode)).length;
    const workerCount = Math.max(1, Math.min(coopAccountSlots, realGroupCount || 1));
    for (let slot = 0; slot < workerCount; slot += 1) {
      void runOrderWorker(slot);
    }
  }

  async function runOrderWorker(accountSlot: number) {
    for (;;) {
      const i = nextRealOrderIndexRef.current;
      nextRealOrderIndexRef.current += 1;
      if (i >= storeGroups.length) return;
      const group = storeGroups[i];
      groupAccountSlotsRef.current[group.key] = accountSlot;
      if (group.chain === "coop" && !group.terminalCode) {
        setOrderProgress((prev) => ({
          ...prev,
          [group.key]: { groupKey: group.key, phase: "failed", error: t("Cửa hàng Co.opmart chưa có terminal_code.") },
        }));
        setResults((prev) => ({ ...prev, [group.key]: "err" }));
        continue;
      }
      const orderGroup = toOrderGroup(group);
      const executor = orderExecutors.find((item) => item.supports(orderGroup));
      if (!executor) continue;
      try {
        const result = await executor.execute(
          orderGroup,
          group.chain === "coop" ? coopOrderContext(group, accountSlot) : { name, phone, address, payMethod, slot: slotOf(group.key), accountSlot },
          (progress) => {
            setOrderProgress((prev) => ({ ...prev, [group.key]: progress }));
            setAgentKis((prev) => {
              const next = [...prev];
              next[i] = progressStepIndex(group, progress.phase);
              return next;
            });
          },
        );
        setOrderProgress((prev) => ({ ...prev, [group.key]: result }));
        if (result.phase === "completed") {
          setResults((prev) => ({ ...prev, [group.key]: "ok" }));
          setAgentKis((prev) => {
            const next = [...prev];
            next[i] = (agentPlan[i]?.steps.length ?? 1) - 1;
              return next;
            });
          continue;
        } else if (result.phase === "awaiting_payment") {
          setAgentKis((prev) => {
            const next = [...prev];
            next[i] = progressStepIndex(group, "awaiting_payment");
            return next;
          });
        } else if (result.phase === "ready_to_confirm") {
          setAgentKis((prev) => {
            const next = [...prev];
            next[i] = progressStepIndex(group, "ready_to_confirm");
            return next;
          });
        }
        return;
  ...
  async function confirmRealOrder(group: StoreGroup, index: number) {
    const prepared = orderProgress[group.key];
    if (!prepared || prepared.phase !== "ready_to_confirm") return;
    const orderGroup = toOrderGroup(group);
    const executor = orderExecutors.find((item) => item.supports(orderGroup));
    if (!executor?.confirm) return;
    try {
      const accountSlot = groupAccountSlotsRef.current[group.key] ?? 0;
      const result = await executor.confirm(
        ...
        (progress) => {
          ...
          setAgentKis((prev) => {
            const next = [...prev];
            next[index] = progressStepIndex(group, progress.phase);
            return next;
          });
        },
      );
      ...
      if (result.phase === "completed") {
        ...
        setAgentKis((prev) => {
          const next = [...prev];
          next[index] = (agentPlan[index]?.steps.length ?? 1) - 1;
          return next;
        });
        void runOrderWorker(accountSlot);
        return;
      }
      if (result.phase === "awaiting_payment" && result.paymentUrl && payMethod === "qr") {
        await openRealPaymentQr(group, index, result);
      } else if (result.phase === "awaiting_payment" && result.paymentUrl && payMethod === "card") {
        await openRealCardPaymentScreen(group, index, result);
      }
    } catch (err) {
      setOrderProgress((prev) => ({
        ...prev,
  ...
  function markPaymentTransferred(group: StoreGroup, index: number) {
    const progress = orderProgress[group.key];
    if (!progress || (progress.phase !== "awaiting_payment" && progress.phase !== "fetching_payment_qr")) return;
    clearPaymentStream(group.key);
    const accountSlot = groupAccountSlotsRef.current[group.key] ?? 0;
    setOrderProgress((prev) => ({
      ...prev,
      [group.key]: { ...progress, phase: "completed" },
    }));
    setResults((prev) => ({ ...prev, [group.key]: "ok" }));
    setAgentKis((prev) => {
      const next = [...prev];
  ...
  async function openRealPaymentQr(group: StoreGroup, index: number, progress: StoreOrderProgress) {
    if (!progress.paymentUrl) return;
    paymentWsRef.current.get(group.key)?.close();
    setOrderProgress((prev) => ({ ...prev, [group.key]: { ...progress, phase: "fetching_payment_qr" } }));
    setAgentKis((prev) => {
      const next = [...prev];
      next[index] = progressStepIndex(group, "fetching_payment_qr");
      return next;
    });
    const sessionId = `cart-coop-qr-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const ws = new WebSocket(await getAgentWsUrl(sessionId, "coop"));
    paymentWsRef.current.set(group.key, ws);
    const timeout = window.setTimeout(() => {
      setOrderProgress((prev) => ({
        ...prev,
        [group.key]: { ...progress, phase: "failed", error: t("Không lấy được mã QR VNPAY thật. Kiểm tra agent-server rồi thử lại.") },
      }));
      ws.close();
    }, 45000);
    ws.onopen = () => {
      ws.send(JSON.stringify({
        ...
        paymentUrl: progress.paymentUrl,
        orderCode: progress.orderCode,
        paymentMethodCode: progress.paymentMethodCode,
        paymentMethodName: progress.paymentMethodName,
      }));
    };
    ws.onmessage = (event) => {
      let message: { type?: string; phase?: string; qrImageBase64?: string; qrContentType?: string; error?: string; orderCode?: string };
      try {
      ...
      if (message.type === "status" && message.phase === "coop_payment_qr_ready" && message.qrImageBase64) {
        ...
        setOrderProgress((prev) => ({
          ...
          [group.key]: {
            ...
            qrImage: `data:${message.qrContentType || "image/png"};base64,${message.qrImageBase64}`,
          },
        }));
      } else if (message.type === "status" && (message.phase === "failed" || message.phase === "coop_payment_failed")) {
        window.clearTimeout(timeout);
        setOrderProgress((prev) => ({
          ...prev,
          [group.key]: { ...progress, phase: "failed", error: message.error || t("Không lấy được mã QR VNPAY.") },
        }));
        ws.close();
      } else if (message.type === "status" && message.phase === "completed") {
        window.clearTimeout(timeout);
        const orderCode = message.orderCode || progress.orderCode;
        setOrderProgress((prev) => ({ ...prev, [group.key]: { ...progress, phase: "completed", orderCode } }));
        setResults((prev) => ({ ...prev, [group.key]: "ok" }));
        setAgentKis((prev) => {
          const next = [...prev];
          next[index] = (agentPlan[index]?.steps.length ?? 1) - 1;
        ...
        clearPaymentStream(group.key);
        void runOrderWorker(groupAccountSlotsRef.current[group.key] ?? 0);
      }
    };
    ws.onerror = () => {
      window.clearTimeout(timeout);
      setOrderProgress((prev) => ({
        ...prev,
        [group.key]: { ...progress, phase: "failed", error: t("Mất kết nối agent-server khi lấy mã QR VNPAY thật.") },
      }));
    };
    ws.onclose = () => window.clearTimeout(timeout);
  }

  async function openRealCardPaymentScreen(group: StoreGroup, index: number, progress: StoreOrderProgress) {
    if (!progress.paymentUrl) return;
    paymentWsRef.current.get(group.key)?.close();
    setPaymentStreams((prev) => ({
      ...prev,
      [group.key]: { status: t("Đang mở màn hình thanh toán thẻ…") },
    }));
    const sessionId = `cart-coop-card-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const ws = new WebSocket(await getAgentWsUrl(sessionId, "coop"));
    paymentWsRef.current.set(group.key, ws);
    let opened = false;
    ...
    const buildMessage = () => ({
      ...
      paymentMethodCode: progress.paymentMethodCode,
      paymentMethodName: progress.paymentMethodName,
      paymentCard: coopPaymentCardPayload(),
    });
    const sendCommand = () => {
    ...
    const retry = window.setInterval(() => {
      if (!opened || acknowledged || attempts >= 10) return;
      sendCommand();
    }, 1200);
    ws.onopen = () => {
      opened = true;
      setPaymentStreams((prev) => ({
        ...prev,
        [group.key]: { ...prev[group.key], status: t("Đã kết nối agent-server, đang chờ trình duyệt sẵn sàng…") },
      }));
      window.setTimeout(sendCommand, 250);
    };
    ws.onmessage = (event) => {
      let message: { type?: string; data?: string; width?: number; height?: number; sourceX?: number; sourceY?: number; message?: string; phase?: string; error?: string; orderCode?: string; orderUrl?: string };
      try {
        message = JSON.parse(String(event.data));
      } catch {
        return;
      ...
      if (message.type === "screencast" && message.data) {
        setPaymentStreams((prev) => ({
          ...
          [group.key]: {
            ...prev[group.key],
            frame: `data:image/jpeg;base64,${message.data}`,
            width: message.width,
            height: message.height,
            sourceX: Number.isFinite(Number(message.sourceX)) ? Number(message.sourceX) : 0,
            sourceY: Number.isFinite(Number(message.sourceY)) ? Number(message.sourceY) : 0,
          },
        }));
      }
      if (message.type === "log" && message.message) {
        if (/Co\.opmart|Co\.op|VNPay|thẻ|the|card/i.test(message.message)) {
          acknowledged = true;
          window.clearInterval(retry);
        }
        setPaymentStreams((prev) => ({
          ...prev,
          [group.key]: { ...prev[group.key], status: message.message },
        }));
      }
      if (message.type === "status" && message.phase === "coop_payment_ready") {
        ...
        setPaymentStreams((prev) => ({
          ...prev,
          [group.key]: { ...prev[group.key], status: t("Màn hình thanh toán đã sẵn sàng. Agent đang tự fill/bấm tiếp nếu đủ thông tin thẻ.") },
        }));
      }
      if (message.type === "status" && (message.phase === "failed" || message.phase === "coop_payment_failed")) {
        acknowledged = true;
        window.clearInterval(retry);
        setPaymentStreams((prev) => ({
          ...prev,
          [group.key]: { ...prev[group.key], status: message.error || t("Thanh toán thẻ chưa hoàn tất, bạn có thể thao tác trực tiếp trên stream.") },
        }));
      }
      if (message.type === "status" && message.phase === "completed") {
        window.clearInterval(retry);
        const orderCode = message.orderCode || progress.orderCode;
        setOrderProgress((prev) => ({ ...prev, [group.key]: { ...progress, phase: "completed", orderCode, paymentUrl: message.orderUrl || progress.paymentUrl } }));
        setResults((prev) => ({ ...prev, [group.key]: "ok" }));
        setAgentKis((prev) => {
          const next = [...prev];
          next[index] = (agentPlan[index]?.steps.length ?? 1) - 1;
    ...
    ws.onerror = () => {
      setPaymentStreams((prev) => ({
        ...prev,
        [group.key]: { ...prev[group.key], status: t("Chưa kết nối được agent-server để mở màn hình thanh toán thẻ.") },
      }));
    };
    ws.onclose = () => {
      window.clearInterval(retry);
      paymentWsRef.current.delete(group.key);
    };
  }
  ...
  useEffect(() => {
    ...
    agentPlan.forEach((sp, i) => {
      ...
      if (hasRealExecutor(sp.group) || (sp.group.chain === "coop" && !sp.group.terminalCode)) {
  ...
  useEffect(() => () => { paymentWsRef.current.forEach((ws) => ws.close()); }, []);
  ...
  function sendPaymentFrameEvent(groupKey: string, payload: Record<string, unknown>) {
    const ws = paymentWsRef.current.get(groupKey);
    if (!ws || ws.readyState !== WebSocket.OPEN) return;
    ws.send(JSON.stringify(payload));
  }

  function clearPaymentStream(groupKey: string) {
    paymentWsRef.current.get(groupKey)?.close();
    paymentWsRef.current.delete(groupKey);
    setPaymentStreams((prev) => {
  ...
  function getPaymentFrameRatios(event: React.MouseEvent<HTMLImageElement> | React.WheelEvent<HTMLImageElement>) {
  ...
  function handlePaymentFrameMouseDown(event: React.MouseEvent<HTMLImageElement>, groupKey: string) {
    event.currentTarget.focus();
    const ratios = getPaymentFrameRatios(event);
    if (!ratios) return;
    paymentPointerDownRef.current = true;
    sendPaymentFrameEvent(groupKey, { type: "frame_mousedown", ...ratios });
  }

  function handlePaymentFrameMouseMove(event: React.MouseEvent<HTMLImageElement>, groupKey: string) {
    if (!paymentPointerDownRef.current) return;
    const now = Date.now();
    if (now - lastPaymentMoveAtRef.current < 50) return;
    lastPaymentMoveAtRef.current = now;
    const ratios = getPaymentFrameRatios(event);
    if (!ratios) return;
    sendPaymentFrameEvent(groupKey, { type: "frame_mousemove", ...ratios });
  }

  function handlePaymentFrameMouseUp(event: React.MouseEvent<HTMLImageElement>, groupKey: string) {
    if (!paymentPointerDownRef.current) return;
    paymentPointerDownRef.current = false;
    const ratios = getPaymentFrameRatios(event);
    if (!ratios) return;
    sendPaymentFrameEvent(groupKey, { type: "frame_mouseup", ...ratios });
  }

  function handlePaymentFrameWheel(event: React.WheelEvent<HTMLImageElement>, groupKey: string) {
    const ratios = getPaymentFrameRatios(event);
    if (!ratios) return;
    sendPaymentFrameEvent(groupKey, {
      type: "frame_wheel",
      ...ratios,
      deltaX: event.deltaX,
      deltaY: event.deltaY,
  ...
  function handlePaymentFrameKeyDown(event: React.KeyboardEvent<HTMLImageElement>, groupKey: string) {
    if (event.metaKey || event.ctrlKey || event.altKey) return;
    const key = event.key;
    if (!key) return;
    if (key.length === 1 || ["Backspace", "Delete", "Tab", "Enter", "Escape", "ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"].includes(key)) {
      if (key !== "Tab") event.preventDefault();
      sendPaymentFrameEvent(groupKey, { type: "frame_keypress", key });
    }
  }

  const activePaymentStreamEntry = Object.entries(paymentStreams).find(([, stream]) => stream.frame || stream.status);
  const activePaymentStreamKey = activePaymentStreamEntry?.[0] ?? "";
  const activePaymentStream = activePaymentStreamEntry?.[1];
  const activePaymentGroup = activePaymentStreamKey ? storeGroups.find((group) => group.key === activePaymentStreamKey) : undefined;
  const showPaymentSideStream = phase === "agent" && payMethod === "card" && Boolean(activePaymentStream);
  ...
  return (
    ...
    >
      ...
      {/* Panel — liquid glass */}
      <div className={`liquid-glass relative flex w-full flex-col rounded-3xl max-h-[85vh] sm:max-h-[90vh] ${showPaymentSideStream ? "max-w-6xl" : "max-w-lg"}`}>
        {/* Header */}
        ...
        <div
          ref={bodyRef}
          className={`flex-1 px-4 pb-4 pt-3 ${showPaymentSideStream ? "grid min-h-0 gap-4 overflow-hidden lg:grid-cols-[minmax(390px,520px)_minmax(0,1fr)]" : "overflow-y-auto"}`}
        >
          {phase === "done" ? (
            ...
            /* ── Trợ lý đặt TUẦN TỰ theo từng checkout độc lập.
                  Co.op dừng ở nút xác nhận, sau đó mới lấy QR thật từ cổng thanh toán. ── */
            <>
              <div className={showPaymentSideStream ? "min-h-0 overflow-y-auto pr-1" : ""}>
                <div className="mb-3 rounded-xl border border-amber-200 bg-amber-50/60 px-3 py-2.5 text-xs text-amber-800">
                  🤖 {coopAccountSlots > 1
                    ? t("Affree có {n} tài khoản Co.op khả dụng nên có thể xử lý nhiều cửa hàng cùng lúc — mỗi tài khoản xong thanh toán mới nhận cửa hàng tiếp theo.", { n: coopAccountSlots })
                    : t("Affree đang dùng 1 tài khoản Co.op nên đặt tuần tự — xác nhận/xong thanh toán cửa hàng này mới chuyển sang cửa hàng tiếp theo.")}
                </div>
                {agentPlan.map((sp, i) => {
                const g = sp.group;
                const ki = agentKis[i] ?? 0;
                ...
                const progress = orderProgress[g.key];
                const stream = paymentStreams[g.key];
                const realGroup = hasRealExecutor(g);
                const queuedReal = realGroup && !progress;
                const waitingPay = !doneStore && Boolean(curStep?.pause) && !(curStep?.pause === "pay-card" && cardConfirmed);
                return (
                  <section
                    key={g.key}
                    className={`mb-3 rounded-2xl border p-3 ${doneStore || queuedReal ? "border-slate-200" : "border-emerald-300"}`}
                  >
                    <div className="mb-2 flex items-center gap-2">
                      ...
                      {doneStore ? (
                        ...
                        <span className="text-xs font-medium text-slate-400">{t("Chờ tới lượt…")}</span>
                      ) : progress?.phase === "failed" ? (
                        <span className="text-xs font-medium text-rose-600">{t("Lỗi")}</span>
                      ) : progress?.phase === "ready_to_confirm" ? (
                        <span className="text-xs font-medium text-blue-600">{t("Chờ xác nhận…")}</span>
                      ) : progress?.phase === "fetching_payment_qr" ? (
                        <span className="text-xs font-medium text-amber-600">{t("Đang lấy QR thật…")}</span>
                      ) : progress?.phase === "awaiting_payment" && progress.paymentUrl ? (
                        <span className="text-xs font-medium text-amber-600">
                          {payMethod === "card" ? t("Đang thanh toán thẻ…") : t("Chờ thanh toán…")}
                        </span>
                      ) : waitingPay ? (
                        <span className="text-xs font-medium text-amber-600">
                    ...
                    <div className="mb-2 flex items-stretch gap-3">
                      ...
                      {payMethod === "qr" && (!realGroup || progress?.qrImage || progress?.phase === "fetching_payment_qr" || progress?.paymentUrl) && (
                        <div className="flex w-28 shrink-0 flex-col items-center justify-center gap-1 rounded-xl border border-slate-200 bg-white p-2">
                          {realGroup ? (
                            progress?.qrImage ? (
                              // eslint-disable-next-line @next/next/no-img-element
                              <img src={progress.qrImage} alt={t("QR thanh toán")} className="w-full rounded bg-white object-contain" />
                            ) : (
                              <div className="flex aspect-square w-full items-center justify-center rounded bg-slate-50 text-center text-[10px] font-medium text-slate-400">
                                {progress?.phase === "fetching_payment_qr" ? t("Đang lấy QR…") : t("Chưa có QR")}
                              </div>
                            )
                          ) : (
                            <div data-qr={g.key} className="w-full rounded bg-white">
                              <QRCode
                                value={`Chuyen khoan: ${g.storeName} | So tien: ${g.total} VND | SDT: ${phone} | ${g.key}`}
                                size={80}
                                level="M"
                          ...
                          {!realGroup && (
                            <button
                              type="button"
                              onClick={() => downloadQr(g.key, g.storeName)}
                    ...
                    {(
                      <div className="space-y-1.5">
                        {queuedReal && (
                          <p className="rounded-lg bg-slate-50 px-2 py-1.5 text-xs font-medium text-slate-500">
                            {t("Đang chờ cửa hàng trước hoàn tất.")}
                          </p>
                        )}
                        {progress?.phase === "failed" && (
                          <p className="rounded-lg bg-rose-50 px-2 py-1.5 text-xs font-medium text-rose-700">
                            {progress.error || t("Co.op không thể tạo đơn cho cửa hàng này.")}
                          </p>
                        )}
                        {progress?.phase === "ready_to_confirm" && (
                          <button
                            type="button"
                            onClick={() => void confirmRealOrder(g, i)}
                            className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-700"
                          >
                            {t("Xác nhận đặt hàng tại {chain}", { chain: g.storeName })}
                          </button>
                        )}
                        {progress?.phase === "fetching_payment_qr" && (
                          <div className="space-y-2 rounded-lg bg-amber-50 px-2 py-1.5 text-xs font-medium text-amber-700">
                            <p>{t("Đang mở trang VNPAY để lấy ảnh QR thật…")}</p>
                            <button
                              type="button"
                              onClick={() => markPaymentTransferred(g, i)}
                              className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-700"
                            >
                              {t("Đã chuyển khoản")}
                            </button>
                        ...
                        {progress?.phase === "awaiting_payment" && progress.paymentUrl && (
                          <div className="space-y-2 rounded-lg bg-amber-50 px-2 py-1.5 text-xs font-medium text-amber-700">
                            <p>
                              {payMethod === "card"
                                ? t("Stream thanh toán thẻ đang mở bên phải. Agent sẽ tự fill/bấm tiếp nếu đủ thông tin, hoặc bạn thao tác trực tiếp trên stream.")
                                : progress.qrImage
                                ? t("QR VNPAY thật đã sẵn sàng. Bạn có thể bấm đã chuyển khoản để mô phỏng hoàn tất.")
                                : t("Đơn Co.op đã tạo, có thể bấm đã chuyển khoản để mô phỏng hoàn tất.")}
                            </p>
                            <button
                              type="button"
                              onClick={() => markPaymentTransferred(g, i)}
                              className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-700"
                            >
                              {payMethod === "card" ? t("Đã thanh toán thẻ") : t("Đã chuyển khoản")}
                            </button>
                          </div>
                        )}
                        {sp.steps.map((st, k) => {
                          if (queuedReal) return null;
              ...
              {showPaymentSideStream && (
                <aside className="hidden min-h-0 overflow-hidden rounded-2xl border border-slate-200 bg-slate-950 shadow-inner lg:flex lg:flex-col">
                  <div className="flex shrink-0 items-center justify-between gap-3 border-b border-white/10 px-3 py-2">
                    <div className="min-w-0">
                      <p className="truncate text-xs font-semibold text-white">
                        {activePaymentGroup?.storeName || t("Màn hình thanh toán thẻ")}
                      </p>
                      <p className="truncate text-[11px] text-slate-300">
                        {activePaymentStream?.status || t("Đang mở stream thanh toán…")}
                      </p>
                      <p className="text-[10px] text-emerald-200">
                        {t("Có thể click/gõ trực tiếp trên màn hình này.")}
                      </p>
                    </div>
                    {activePaymentGroup && (
                      <button
                        type="button"
                        onClick={() => markPaymentTransferred(activePaymentGroup, storeGroups.findIndex((group) => group.key === activePaymentGroup.key))}
                  ...
                  <div className="flex min-h-0 flex-1 items-center justify-center bg-black p-3">
                    {activePaymentStream?.frame ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img
                        src={activePaymentStream.frame}
                        alt={t("Màn hình thanh toán thẻ")}
                        tabIndex={0}
                        role="application"
                        onMouseDown={(event) => handlePaymentFrameMouseDown(event, activePaymentStreamKey)}
                        onMouseMove={(event) => handlePaymentFrameMouseMove(event, activePaymentStreamKey)}
                        onMouseUp={(event) => handlePaymentFrameMouseUp(event, activePaymentStreamKey)}
                        onMouseLeave={(event) => handlePaymentFrameMouseUp(event, activePaymentStreamKey)}
                        onWheel={(event) => handlePaymentFrameWheel(event, activePaymentStreamKey)}
                        onKeyDown={(event) => handlePaymentFrameKeyDown(event, activePaymentStreamKey)}
                        className="max-h-full max-w-full cursor-crosshair select-none rounded-lg object-contain shadow-2xl outline-none ring-0 focus:ring-2 focus:ring-emerald-400"
                        draggable={false}
                      />
                    ) : (
                      <div className="flex h-full w-full items-center justify-center rounded-lg border border-white/10 bg-slate-900 text-sm font-medium text-slate-400">
                        {t("Đang chờ frame từ agent-server…")}
                      </div>
                    )}
                  </div>
                </aside>
            ...
            <>
              ...
              {hasCoopGroups && (
                <section className="mb-3 rounded-2xl border border-emerald-100 bg-emerald-50/40 p-3">
                  <h3 className="mb-2.5 text-xs font-semibold uppercase tracking-wide text-emerald-700">
                    {t("Thông tin Co.opmart")}
                  </h3>
                  <Field label={t("Email nhận hóa đơn / thanh toán Co.op")}>
                    <input
                      type="email"
                      value={coopEmail}
                  ...
                  <div className="mt-2 rounded-xl bg-white/70 px-3 py-2 text-[11px] text-slate-600 ring-1 ring-emerald-100">
                    <p className="font-medium text-slate-700">{t("Địa chỉ Co.op tự tách từ ô địa chỉ:")}</p>
                    <p className="mt-1">
                      {t("Số nhà/đường")}: <span className="font-medium">{coopAddressParts.addressLine || "—"}</span>
                    </p>
                    <p>
                      {t("Phường/Xã")}: <span className="font-medium">{coopAddressParts.wardName || "—"}</span>
                      {" · "}
                      {t("Quận/Huyện")}: <span className="font-medium">{coopAddressParts.districtName || "—"}</span>
                    </p>
                    <p>
                      {t("Tỉnh/TP")}: <span className="font-medium">{coopAddressParts.provinceName || "—"}</span>
                    </p>
                  </div>
                </section>
              )}
                ...
                >
                        ...
                        <div className="flex shrink-0 items-center gap-1">
                          <button
                            onClick={() =>
                              ...
                              item.qty <= 1
                                ? onRemove(item.product.id, item.offer.store.id)
                                : onUpdateQty(item.product.id, item.offer.store.id, item.qty - 1)
                            }
                          ...
                          <button
                            onClick={() => onUpdateQty(item.product.id, item.offer.store.id, item.qty + 1)}
                            className="flex h-7 w-7 items-center justify-center rounded-full border border-slate-200 bg-white text-sm font-bold text-slate-600 hover:border-slate-300 hover:bg-slate-100"
                          >
                            +
                          </button>
                          ...
                          <button
                            onClick={() => onRemove(item.product.id, item.offer.store.id)}
                  ...
                  <div className="space-y-2">
                    ...
                    <Field label={t("Ghi chú")}>
                      <textarea
                        value={storeExtras[group.key]?.note || ""}
                        onChange={(e) => setExtra(group.key, "note", e.target.value)}
                        rows={2}
                ...
                <div className="space-y-2">
                  ...
                  {activeCardNumber.replace(/\D/g, "").length >= 12 ? (
                    ...
                    <p className="rounded-lg bg-amber-50 px-3 py-2 text-[11px] font-medium text-amber-700">
                      {t("Thẻ đã lưu chỉ có 4 số cuối nên không đủ để auto-fill VNPAY. Hãy dùng thẻ khác để nhập full số + CVV.")}
                    </p>
                  )}

                  <div className="flex items-center justify-between gap-2">
                    <p className={`text-[11px] ${cardReady ? "text-slate-400" : "text-amber-600"}`}>
                      {cardReady
                        ? "✓ " + t("{card} sẽ được trợ lý dùng thanh toán tự động.", { card: maskedCardLabel })
                        : t("Nhập CVV hoặc dùng thẻ khác để trợ lý tự fill/bấm tiếp.")}
                    </p>
                    <button
                      type="button"
        ...
        {phase === "cart" && (
          <div className="shrink-0 border-t border-slate-100 px-4 py-3">
            ...
            <button
              ...
              disabled={!phone.trim() || !address.trim() || !paymentReady || Boolean(missingTerminalError)}
