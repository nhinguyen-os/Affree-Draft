...
import { getCoopAccount, getCoopAccounts } from "../../../../agent-server/account-sheet";
...
async function affreeCoopAccount(slot?: number): Promise<{ phone: string; password: string }> {
  try {
    const accounts = await getCoopAccounts().catch(() => null);
    const usable = (accounts ?? []).filter((item: { active?: boolean }) => item.active);
    const pool = usable.length ? usable : (accounts ?? []);
    const account =
      typeof slot === "number" && slot >= 0 && pool.length
        ? pool[slot % pool.length]
        : await getCoopAccount();
...
export async function POST(req: NextRequest) {
  ...
  try {
    if (action === "accountStatus") {
      const accounts = await getCoopAccounts();
      const activeAccounts = accounts.filter((account: { active?: boolean }) => account.active);
      return NextResponse.json({
        ok: true,
        total: accounts.length,
        active: activeAccounts.length || accounts.length,
      });
    }
    ...
    if (action === "register" || action === "createCart") {
      ...
      const requestedSlot = Number(body.accountSlot);
      const affree = await affreeCoopAccount(Number.isFinite(requestedSlot) ? requestedSlot : undefined);
      if (affree) {
