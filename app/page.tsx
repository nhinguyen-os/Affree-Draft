...
/**
 * 1 dòng "Có gì mới":
 * - Ô sheet có XUỐNG DÒNG THẬT (Alt+Enter) → giữ nguyên format y như sheet: mỗi dòng 1 hàng.
 * - Không có xuống dòng: nếu mục gộp các mục con "N.M." (vd mục 10 gộp 10.1…10.5) thì TÁCH
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Đổi URL bằng history.replaceState thay vì router.replace: "/" (app/page.tsx) và
    // "/p/…" ([...slug]) là 2 ROUTE KHÁC NHAU — đi qua router sẽ unmount + remount toàn bộ
    // Home (load lại từ đầu, rất chậm). replaceState chỉ đổi thanh địa chỉ, giữ nguyên cây
    // React → mở/đóng sản phẩm tức thì. Đọc window.location.pathname (không dùng usePathname
    // vì giá trị đó không cập nhật theo replaceState).
    const hostSub = getHostSub();
    const curPath = window.location.pathname;
    if (hostSub && curPath === "/" && (url === `/${hostSub}` || shortUrl === `/${hostSub}`)) return;
    if (curPath !== url && curPath !== shortUrl) window.history.replaceState(null, "", url);
  }, [urlSynced, selected, activeTep, activeBrand, activeCat, activeChain]);
  ...
  async function recordBuy(o: RankedOffer, note?: string, orderCode?: string, storeOrderCode?: string, slot?: string, pay?: "qr" | "card" | "cod") {
  ...
  // Click tên cửa hàng trong các FORM ĐẶT HÀNG → mở trang bán hàng của cửa hàng đó.
  // Ưu tiên mở đúng cửa hàng (offers lọc theo storeId); nếu catalog không có offer gắn id
  // này (id GIS khác id kiểu sheet) thì fallback mở trang theo CHUỖI (__chain__, như onDealStore).
  function openStorePage(store: Store) {
    const cat = rawCatalog || catalog;
    const hasDirect = !!cat?.offers.some((o) => o.storeId === store.id);
    if (hasDirect || !store.chain) setStoreProducts(store);
    else setStoreProducts({ ...store, id: `__chain__${store.chain}`, name: store.name || chainLabel(store.chain) } as Store);
  }
        ...
        <div className="mx-auto max-w-6xl px-3 py-3 sm:px-4">
          <div className="flex items-center gap-2 sm:gap-3">
            <Link
              href="/"
              onClick={(e) => {
                // CHẶN điều hướng route: "/" và "/p/…" là 2 route khác nhau, đi qua router sẽ
                // remount toàn bộ Home (chậm). Tự reset state + replaceState về "/" — tức thì.
                e.preventDefault();
                window.scrollTo(0, 0);
                // Đóng MỌI overlay che trang (phiên đặt hộ, giỏ hàng, trang cửa hàng) và reset
                // selected ĐỒNG BỘ — trước đây setSelected nằm trong startTransition (chạy trễ)
                // nên effect state→URL kịp đẩy ngược về /p/… → logo không về được trang chủ.
                setCobrowse(null);
                setCartOpen(false);
                setStoreProducts(null);
                setSelected(null);
                window.history.replaceState(null, "", "/");
                // Các filter còn lại clear ở priority thấp — section nặng render dần sau.
                startTransition(() => {
                  setActiveTep(null);
                  setActiveCat(null);
                  setActiveBrand(null);
...

      {
        ...
        cobrowse && (
          <CobrowseSession
            ...
            onPlaced={(store, code, slot, pay) => {
              const o = cobrowse.offersByKey[store.key];
              // Mã CHUNG của lần mua (gom lịch sử) + mã RIÊNG + khung giờ giao + hình thức thanh toán của cửa hàng vừa đặt.
              if (o) recordBuy(o, undefined, cobrowse.orderCode, code, slot, pay);
...

      {
        buyTui && (
          <TuiAgentModal
            ...
            onOpenStore={(chain, label) => {
              setBuyTui(null);
              setStoreProducts({ id: `__chain__${chain}`, name: label || chainLabel(chain as Chain), chain: chain as Chain } as Store);
            }}
            onPlaced={() => {
              recordTuiBuy(buyTui);
        ...
        buyOffer && !isWalmartOffer(buyOffer) && (
          isPnjOffer(buyOffer) && process.env.NEXT_PUBLIC_PNJ_REAL ? (
            ...
            <CoopOrderAgentModal
              ...
              onPlaced={(code, chosen, _note, pay) => {
                recordBuy(chosen, undefined, code, undefined, undefined, pay ?? undefined);
