...
import { chainColor, chainLabel, chainMinOrder, findChainBySlug, getStore, getStores, physicalStoresOfChain, setDynamicMinOrders, setDynamicSourceLogos, setDynamicSourceNames, setDynamicStores, setStoreCurrencies, sourceLabelFromStoreId, storeCurrency } from "@/lib/stores";
...
import { storeOrderFromOffer, storeOrdersFromCart } from "@/lib/cobrowse-build";
import type { CobrowseStore } from "@/components/CobrowseSession";
...
const PNJOrderModal = dynamic(() => import("@/components/PNJOrderModal"), { ssr: false });
...
const CobrowseSession = dynamic(() => import("@/components/CobrowseSession"), { ssr: false });
...
function isPnjOffer(offer: RankedOffer | null | undefined) {
  if (!offer) return false;
  const hay = [
    offer.store.chain,
    offer.store.id,
  ...
  return hay.includes("pnj");
}

/**
 * Một checkout Co.op chỉ được phép dùng một terminal. Không dùng slug `coop-...`
 * làm fallback vì slug cửa hàng (vd `coop-binh-tan`) không phải terminal hợp lệ.
 * Giữ fallback cho catalog cũ từng lưu id dạng `coop-151_sgc`.
 */
function coopTerminalCodeOf(offer: RankedOffer): string {
  const mapped = offer.store.terminalCode?.trim();
  if (mapped) return mapped;
  const legacy = offer.store.id.replace(/^coop-/, "").trim();
  return /^\d+_[a-z0-9-]+$/i.test(legacy) ? legacy : "";
}
...
import { CATEGORY_GROUPS, GROUP_TILE, DEFAULT_TILE_EMOJIS, DEFAULT_TILE_TINTS, categoryGroup, buildCategoryTiles, CAT_EMOJI, type ServiceTile } from "@/lib/categories";
...
/**
 * 1 dòng "Có gì mới": nếu mục gộp các mục con "N.M." (vd mục 10 gộp 10.1…10.5) thì TÁCH
 * mỗi mục con XUỐNG DÒNG cho dễ đọc. Khoá theo đúng số mục cha `num` để KHÔNG nhầm với ngày
 * (24.06.2026) hay giá. Không có mục con → hiện nguyên văn.
 */
function HighlightText({ text, num }: { text: string; num: number }) {
  const parts = text.split(new RegExp(`\\s+(?=${num}\\.\\d+[.)])`));
  if (parts.length <= 1) return <>{text}</>;
  return (
    <span className="flex flex-col gap-0.5">
      {parts.map((p, i) => (
        <span key={i} className={i === 0 ? "" : "pl-1"}>{p}</span>
      ))}
    </span>
  );
}
...
export default function Home() {
  ...
  // Hàng đợi terminal khi checkout thật Co.op từ giỏ. Mỗi terminal = một đơn riêng.
  const [coopCartTerminalQueue, setCoopCartTerminalQueue] = useState<string[]>([]);
  const [completedCoopCartTerminals, setCompletedCoopCartTerminals] = useState<string[]>([]);
  const completedCoopCartTerminalsRef = useRef<Set<string>>(new Set());
  ...
  const activeCoopCartTerminal = coopCartTerminalQueue[0] || "";
  const activeCoopCartItems = useMemo(
    () =>
      activeCoopCartTerminal
        ? cartItems.filter(
            (item) => item.offer.store.chain === "coop" && coopTerminalCodeOf(item.offer) === activeCoopCartTerminal,
          )
        : [],
    [activeCoopCartTerminal, cartItems],
  );
  ...
  // Đồng bộ trang cửa hàng (overlay) ↔ URL ?storeid= để F5 KHÔNG mất trang: mở store →
  // ghi ?storeid vào URL (replaceState, không thêm history), F5 thì handler deep-link ở trên
  // tự mở lại. Đóng store → gỡ param. Chờ deepLinkDone để không ghi đè lúc handler đang chạy.
  // Bỏ qua store ảo brand (__brand__…) vì không tra lại được qua /api/store-lookup.
  useEffect(() => {
    if (!deepLinkDone) return;
    const url = new URL(window.location.href);
    const cur = url.searchParams.get("storeid") || url.searchParams.get("storeId");
    if (storeProducts && !storeProducts.id.startsWith("__brand__")) {
      if (cur !== storeProducts.id) {
        url.searchParams.delete("storeId");
        url.searchParams.set("storeid", storeProducts.id);
  ...
  const priceStats = useMemo(() => {
    const m = new Map<string, { min: number; max: number; stores: number; variants: number; outOfStock: boolean; currency: string; soleSource: string; soleUrl: string; minChain: string; soleChain: string; soleStoreId: string; srcChain: string }>();
    if (!catalog) return m;
    for (const p of catalog.products) {
      ...
      if (!inStock.length) {
        ...
        m.set(p.id, { min: 0, max: 0, stores: 0, variants: 0, outOfStock: priced.length > 0, currency: storeCurrency(priced[0]?.storeId), soleSource: "", soleUrl: "", minChain: "", soleChain: "", soleStoreId: "", srcChain: "" });
        continue;
      }
      ...
      // Nhiều nơi bán nhưng TẤT CẢ cùng 1 chuỗi (vd PNJ: 105 store_id PNJ_000xx) → nhãn chuỗi
      // để card hiện "PNJ · N nơi bán" thay vì chỉ "Có N nơi bán". Dựa trên nhãn nguồn suy từ
      // store_id (chainBaseKey) — cùng chuỗi ⇒ cùng nhãn. Khác chuỗi (vd BHX+Co.op) ⇒ rỗng.
      const srcLabels = new Set(storeIds.map((sid) => sourceLabelFromStoreId(sid)).filter(Boolean));
      const srcChain = !sole && srcLabels.size === 1 ? [...srcLabels][0] : "";
      m.set(p.id, {
        min: minPrice,
        max: Math.max(...prices),
  ...
  // Kho "mua thêm" cho trang cửa hàng mô phỏng (co-browse): sản phẩm THẬT khác của
  // CÙNG cửa hàng từ catalog (tối đa 100 — 4 món đầu hiện làm gợi ý, còn lại phục vụ Ô TÌM KIẾM).
  function cobrowseSuggestions(storeId: string, excludeProductIds: string[]) {
    if (!catalog) return [];
    ...
    for (const o of catalog.offers) {
      if (o.storeId !== storeId || o.price <= 0 || !o.inStock) continue;
      if (ex.has(o.productId) || seen.has(o.productId)) continue;
      const p = byId.get(o.productId);
      if (!p) continue;
      seen.add(o.productId);
      out.push({ name: p.name, price: o.price, image: p.image });
  ...
  /** Ghi lịch sử từng dòng sau khi Co.op xác nhận thành công cả nhóm terminal. */
  async function recordCoopCartGroup(items: CartItem[], orderCode: string) {
    for (const item of items) {
      await addPurchase({
        productId: item.product.id,
        productName: item.product.name,
        storeId: item.offer.store.id,
        storeName: item.offer.store.name,
        chain: item.offer.store.chain,
        qty: item.qty,
        unitPrice: item.offer.price,
        buyerLat: userLoc?.lat,
        buyerLng: userLoc?.lng,
        buyerAddr: userAddr || undefined,
        buyerNote: `Co.op order ${orderCode}`,
        buyerPhone: authUser?.phone || getProfile().phone || undefined,
      });
    }
  }
  ...
  /**
   * Đóng modal của terminal hiện tại. Chỉ khi terminal đã đặt thành công mới xóa
   * nhóm đó khỏi giỏ và mở tiếp modal có sẵn cho terminal kế tiếp.
   */
  function closeCoopCartCheckout() {
    const terminalCode = activeCoopCartTerminal;
    const completed = terminalCode && (
      completedCoopCartTerminalsRef.current.has(terminalCode) || completedCoopCartTerminals.includes(terminalCode)
    );
    if (!terminalCode || !completed) {
      setBuyOffer(null);
      setCoopCartTerminalQueue([]);
      setCompletedCoopCartTerminals([]);
      setCoopCartPrefill(null);
      return;
    }

    const remainingTerminals = coopCartTerminalQueue.slice(1);
    const nextTerminal = remainingTerminals[0];
    const nextOffer = nextTerminal
      ? cartItems.find((item) => item.offer.store.chain === "coop" && coopTerminalCodeOf(item.offer) === nextTerminal)?.offer
      : undefined;

    setCartItems((previous) =>
      previous.filter((item) => item.offer.store.chain !== "coop" || coopTerminalCodeOf(item.offer) !== terminalCode),
    );
    setCompletedCoopCartTerminals((previous) => previous.filter((code) => code !== terminalCode));
    completedCoopCartTerminalsRef.current.delete(terminalCode);
    setCoopCartTerminalQueue(remainingTerminals);

    if (nextTerminal && nextOffer) {
      setBuyOffer(nextOffer);
      return;
    }
  ...
  const renderSource = (st: { stores: number; soleSource: string; soleUrl: string; soleChain: string; soleStoreId?: string; srcChain?: string }) => {
    ...
    // Nhiều nơi bán cùng 1 chuỗi → hiện tên chuỗi kèm số nơi (vd "PNJ · 105 nơi bán").
    if (st.srcChain) return <>{t("{chain} · {n} nơi bán", { chain: st.srcChain, n: st.stores })}</>;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-sky-50 to-emerald-50 text-slate-900">
        ...
        <section className={`min-w-0 ${mobileView === "map" ? "hidden lg:block" : ""}`}>
          ...
          {/* #8/#9: GIỮ khi tìm (gợi ý "Giá hời liên quan"); ẨN khi xem 1 danh mục (trang "Xem tất cả"). */}
          {
            !selected && !activeTep && !activeCat && !activeBrand && !activeChain && !quickFilter && (areaDeals.length > 0 || dealsRadiusKm != null) && (
              <NearbyDeals
                rows={areaDeals}
                radiusKm={dealsRadiusKm}
                setRadiusKm={setDealsRadiusKm}
                effKm={dealsEffKm}
                userLoc={userLoc}
                lang={lang}
                query={query}
                onInfo={(p) => setInfoProduct(p)}
                onAdd={(p) => addToCart(p)}
        ...
        cartOpen && cartItems.length > 0 && (
          <CartModal
            ...
            onOrderWithAgent={(_offers, context) => {
              ...
              // Giỏ thường → màn đặt đơn co-browse 2 cột (giả lập, gộp theo cửa hàng).
              const offersByKey: Record<string, RankedOffer> = {};
              for (const it of cartItems) if (!offersByKey[it.offer.store.id]) offersByKey[it.offer.store.id] = it.offer;
              // Gợi ý mua thêm theo TỪNG cửa hàng trong giỏ (sản phẩm thật của cửa hàng đó).
              const suggestionsByStoreId: Record<string, { name: string; price: number; image?: string }[]> = {};
              for (const id of Object.keys(offersByKey)) {
                suggestionsByStoreId[id] = cobrowseSuggestions(id, cartItems.filter((i) => i.offer.store.id === id).map((i) => i.product.id));
              }
              setCobrowse({
                buyer: { name: context.name, phone: context.phone, address: context.address },
                stores: storeOrdersFromCart(cartItems, context.slotsByStoreId, suggestionsByStoreId),
                offersByKey,
      ...
      {
        // Màn đặt đơn co-browse cho GIỎ HÀNG (nhiều cửa hàng).
        cobrowse && (
          <CobrowseSession
            lang={lang}
            buyer={cobrowse.buyer}
            stores={cobrowse.stores}
            startStep="account"
            defaultPay={cobrowse.pay ?? undefined}
            cardLabel={cobrowse.cardLabel ?? undefined}
            onClose={() => {
              setCobrowse(null);
              // Dọn giỏ các cửa hàng đã đặt (nếu có) — giống onOrdered.
              if (orderedStoreIds.length) {
                const ok = new Set(orderedStoreIds);
                setCartItems((prev) => prev.filter((i) => !ok.has(i.offer.store.id)));
                setOrderedStoreIds([]);
      ...
      {
        buyOffer && !isWalmartOffer(buyOffer) && (
          isPnjOffer(buyOffer) && process.env.NEXT_PUBLIC_PNJ_REAL ? (
            <PNJOrderModal
              offer={buyOffer}
              alternatives={(catalog && buyOffer ? rankOffersForProduct(catalog, buyOffer.product, userLoc).filter(isPnjOffer) : [])}
              geoAddr={userAddr}
              onClose={() => setBuyOffer(null)}
              onPlaced={(code, chosen) => {
                setBuyOffer(null);
            ...
            <CoopOrderAgentModal
              key={activeCoopCartTerminal || `direct-${buyOffer.store.id}`}
              offer={buyOffer}
              lang={lang}
              ...
              defaultQty={cartQtyFor(buyOffer.product.id) || 1}
              sameStoreCartItems={activeCoopCartTerminal ? activeCoopCartItems : []}
              lockedTerminalCode={activeCoopCartTerminal || undefined}
              initialPayMethod={coopCartPrefill?.payMethod ?? null}
              initialSlot={coopCartPrefill?.slot}
              autoStart={Boolean(activeCoopCartTerminal)}
              onClose={closeCoopCartCheckout}
              onPlaced={(code, chosen) => {
                if (activeCoopCartTerminal) {
                  if (completedCoopCartTerminalsRef.current.has(activeCoopCartTerminal)) return;
                  completedCoopCartTerminalsRef.current.add(activeCoopCartTerminal);
                  setCompletedCoopCartTerminals((previous) => Array.from(new Set([...previous, activeCoopCartTerminal])));
                  void recordCoopCartGroup(activeCoopCartItems, code).catch(() => undefined);
                } else {
                  void recordBuy(chosen, undefined, code);
            ...
            // MUA NGAY: giữ FORM GỐC (OrderAgentModal, phase "form") làm BƯỚC 1; bấm xác nhận
            // → onProceed mở màn chọn tài khoản + đặt đơn co-browse 2 cột (dùng chung state cobrowse).
            <ActiveOrderAgentModal
              ...
              onProceed={({ offer, qty, name, phone, address, slot, payMethod, cardLabel }) => {
                setCobrowse({
                  buyer: { name, phone, address },
                  stores: [storeOrderFromOffer(offer, qty, slot, cobrowseSuggestions(offer.store.id, [offer.product.id]))],
                    ...
                    <>
                      <label className="mb-3 block">
                        ...
                        <textarea
                          ...
                          rows={4}
                          autoFocus
                          className="w-full resize-none rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100"
                      ...
                      <label className="mb-2.5 block">
                        ...
                        <input
                          ...
                          className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100"
                  ...
                  {!isLove && (
                    <label className="mb-3 block">
                      ...
                      <textarea
                        ...
                        onChange={(e) => setContactMsg(e.target.value)}
                        rows={3}
                        className="w-full resize-none rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100"
                        placeholder={msgPlaceholder}
                      />
                    </label>
                  )}
        ...
        storeProducts && (rawCatalog || catalog) && (() => {
          ...
          return (
            <StoreProductsPage
              ...
              // "Giá hời quanh đây" dùng chung <NearbyDeals/> — truyền data + handler từ Home
              // xuống để trang cửa hàng KHÔNG có sản phẩm cũng hiện khung này (đỡ trống trải).
              deals={areaDeals}
              dealsRadiusKm={dealsRadiusKm}
              setDealsRadiusKm={setDealsRadiusKm}
              dealsEffKm={dealsEffKm}
                ...
                <div className="space-y-2.5">
                  <label className="block">
                    ...
                    <input
                      ...
                      autoFocus
                      className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-100"
                    />
                  </label>
                  <label className="block">
                  ...
                  <label className="block">
                    ...
                    <textarea
                      ...
                      className="w-full resize-none rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-100"
                    />
                  </label>
                </div>
              </section>
              <section className="mt-3 rounded-2xl border border-slate-400 p-3">
                <div className="space-y-2.5">
                  <div className="flex items-center gap-3">
