...
function mergeCoopDeliveryInfo(primary?: CoopDeliveryInfo, fallback?: CoopDeliveryInfo): CoopDeliveryInfo | undefined {
  if (!primary && !fallback) return undefined;
  return {
    ...(fallback ?? {}),
    ...(primary ?? {}),
    deliveryType: primary?.deliveryType || fallback?.deliveryType,
    name: primary?.name || fallback?.name,
    phone: primary?.phone || fallback?.phone,
    email: primary?.email || fallback?.email,
    addressId: primary?.addressId || fallback?.addressId,
    addressLine: primary?.addressLine || fallback?.addressLine,
    wardId: primary?.wardId || fallback?.wardId,
    wardName: primary?.wardName || fallback?.wardName,
    districtId: primary?.districtId || fallback?.districtId,
    districtName: primary?.districtName || fallback?.districtName,
    provinceId: primary?.provinceId || fallback?.provinceId,
    provinceName: primary?.provinceName || fallback?.provinceName,
    fullAddress: primary?.fullAddress || fallback?.fullAddress,
    ...
    scheduledDeliveryDate: primary?.scheduledDeliveryDate || fallback?.scheduledDeliveryDate,
    scheduledDeliveryTimeSlotFrom: primary?.scheduledDeliveryTimeSlotFrom || fallback?.scheduledDeliveryTimeSlotFrom,
    scheduledDeliveryTimeSlotTo: primary?.scheduledDeliveryTimeSlotTo || fallback?.scheduledDeliveryTimeSlotTo,
  };
}
...
export async function prepareCoopCheckout(input: {
  ...
  let cartToken = confirmation.cartToken;
  let existingDelivery = mergeCoopDeliveryInfo(readConfirmationData(confirmation.data).deliveryInfo, input.deliveryInfo);
  if (!existingDelivery?.fullAddress || !existingDelivery.name || !existingDelivery.phone) {
    const fallbackDelivery = input.deliveryInfo;
    if (fallbackDelivery?.fullAddress && fallbackDelivery.name && fallbackDelivery.phone) {
      const delivery = await updateCoopDeliveryInfo({
        accessToken: input.accessToken,
        terminalCode: input.terminalCode,
        cartToken,
        deliveryInfo: fallbackDelivery,
      });
      cartToken = delivery.cartToken;
      existingDelivery = mergeCoopDeliveryInfo(readConfirmationData(delivery.data).deliveryInfo, fallbackDelivery);
    }
  }
  if (!existingDelivery?.fullAddress || !existingDelivery.name || !existingDelivery.phone) {
    throw new CoopOrderError("Co.op chưa có địa chỉ/người nhận trong giỏ. Vui lòng thêm địa chỉ trên Co.op trước.", {
      ...
      detail: {
        ...
        fallbackDeliveryInfo: input.deliveryInfo,
        confirmationDeliveryInfo: readConfirmationData(confirmation.data).deliveryInfo,
      },
    });
  }
