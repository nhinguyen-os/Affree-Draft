...
- Address synced: ${Boolean(payload.addressSynced)}
- Old cart cleared: ${Boolean(payload.cartCleared)}
...
- Sau khi login, bắt buộc hoàn tất các checkpoint theo thứ tự: xóa/lưu lại địa chỉ addressSynced, xóa sạch giỏ cũ cartCleared, rồi mới mở product URL. Không được bỏ qua checkpoint false sau CAPTCHA/resume.
...
async function submitWalmartPassword(page, payload, password, sendLog, passwordInput) {
  const typePasswordAndVerify = async () => {
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const liveInput = attempt === 0
        ? passwordInput
        : await waitForFirstVisibleLocator(page, walmartPlaybook.SELECTORS.passwordInput, 3000);
      if (!liveInput) continue;
      await liveInput.click({ timeout: 3000 }).catch(() => { });
      await liveInput.press(process.platform === "darwin" ? "Meta+A" : "Control+A").catch(() => { });
      await liveInput.press("Backspace").catch(() => { });
      await liveInput.pressSequentially(password, { delay: 85 }).catch(async () => {
        await liveInput.fill(password);
      });
      await page.waitForTimeout(350);
      const currentValue = await liveInput.inputValue().catch(() => "");
      if (currentValue === password) return true;
    }
    return false;
  };

  if (!await typePasswordAndVerify()) {
    sendLog("Walmart: Ô mật khẩu bị giao diện reset trước khi submit.", "warning");
  ...
  try {
    ...
    if (addrResult.success) {
      payload.addressSynced = true;
      sendLog("Walmart: Đã cập nhật địa chỉ giao hàng thành công.", "success");
    } else {
      sendLog(`Walmart: Không sync được địa chỉ (${addrResult.reason}), dừng fast-path.`, "warning");
      return { state: "fallback" };
    }
  } catch (e) {
  ...
  try {
    ...
    if (!clearResult.success || !clearResult.isEmpty) {
      sendLog(`Walmart: Giỏ hàng vẫn còn sản phẩm (${clearResult.reason || "cart_not_empty"}).`, "warning");
      return { state: "fallback" };
    } else if (clearResult.removed > 0) {
...
async function runWalmartScriptedFastPath(page, payload, sendLog, sendStatus, tracer, sessionLogger, history, totalSteps = 0) {
  ...
  // CAPTCHA can interrupt address deletion or cart cleanup after login. These
  // durable payload checkpoints make every resume restart the unfinished step
  // instead of jumping directly to the product/variant stage.
  if (payload.loginReady && !payload.cartReady) {
    if (!payload.addressSynced) {
      payload.resumeStage = "address_sync";
      sendLog("Walmart: Tiếp tục checkpoint đồng bộ địa chỉ còn dang dở.", "info");
      try {
        const addressResult = await walmartPlaybook.syncDeliveryAddress(page, payload);
        if (!addressResult.success) {
          if (await walmartPageHasCaptcha(page)) {
            const pauseMessage = "Walmart yêu cầu CAPTCHA trong lúc cập nhật địa chỉ. Xử lý xong Affree sẽ làm lại bước địa chỉ.";
            return finishWalmartPause({ page, tracer, sessionLogger, history, reason: "captcha", pauseMessage, sendStatus, totalSteps });
          }
          sendLog(`Walmart: Checkpoint địa chỉ chưa hoàn tất (${addressResult.reason}).`, "warning");
          return null;
        }
        payload.addressSynced = true;
        payload.resumeStage = "cart_clear";
        sendLog("Walmart: Checkpoint địa chỉ đã hoàn tất.", "success");
      } catch (error) {
        if (await walmartPageHasCaptcha(page)) {
          const pauseMessage = "Walmart yêu cầu CAPTCHA trong lúc cập nhật địa chỉ. Xử lý xong Affree sẽ làm lại bước địa chỉ.";
          return finishWalmartPause({ page, tracer, sessionLogger, history, reason: "captcha", pauseMessage, sendStatus, totalSteps });
        }
        sendLog(`Walmart: Lỗi checkpoint địa chỉ (${error.message}).`, "warning");
        return null;
      }
    }

    if (!payload.cartCleared) {
      payload.resumeStage = "cart_clear";
      sendLog("Walmart: Tiếp tục checkpoint xóa giỏ hàng cũ.", "info");
      try {
        const clearResult = await walmartPlaybook.clearCart(page);
        if (!clearResult.success || !clearResult.isEmpty) {
          if (await walmartPageHasCaptcha(page)) {
            const pauseMessage = "Walmart yêu cầu CAPTCHA trong lúc xóa giỏ cũ. Xử lý xong Affree sẽ làm lại bước xóa giỏ.";
            return finishWalmartPause({ page, tracer, sessionLogger, history, reason: "captcha", pauseMessage, sendStatus, totalSteps });
          }
          sendLog(`Walmart: Checkpoint xóa giỏ chưa hoàn tất (${clearResult.reason}).`, "warning");
          return null;
        }
        payload.cartCleared = true;
        payload.resumeStage = "product";
        sendLog(`Walmart: Checkpoint giỏ cũ đã hoàn tất${clearResult.removed ? `, đã xóa ${clearResult.removed} sản phẩm` : ""}.`, "success");
      } catch (error) {
        if (await walmartPageHasCaptcha(page)) {
          const pauseMessage = "Walmart yêu cầu CAPTCHA trong lúc xóa giỏ cũ. Xử lý xong Affree sẽ làm lại bước xóa giỏ.";
          return finishWalmartPause({ page, tracer, sessionLogger, history, reason: "captcha", pauseMessage, sendStatus, totalSteps });
        }
        sendLog(`Walmart: Lỗi checkpoint xóa giỏ (${error.message}).`, "warning");
        return null;
      }
    }
  }
  ...
  if (!payload.cartReady && /\/ip\//i.test(page.url())) {
    ...
    if (cartResult.success) {
      ...
      const pauseMessage = "Walmart đã mở phần thanh toán. Vui lòng xác nhận thông tin thẻ trên Affree để tiếp tục.";
  ...
  if (payload.cartReady) {
    ...
    const pauseMessage = "Walmart đã mở phần thanh toán. Vui lòng xác nhận thông tin thẻ trên Affree để tiếp tục.";
    sendLog("Walmart: Đã xác nhận địa chỉ và mở phần thanh toán.", "success");
    return finishWalmartPause({
      page,
      tracer,
      sessionLogger,
...
async function runAgenticToolUseLoop(page, payload, sendLog, sendStatus, options = {}) {
  ...
  // Login Walmart vẫn do Agentic xử lý để giữ nhịp tương tác hiện tại. Chỉ sau
  // khi đã xác nhận signed-in mới bàn giao sang Playwright deterministic.
  if (isWalmartPayload && (payload.loginReady || payload.cartReady) && !options.skipScriptedFastPath) {
    try {
      const scripted = await runWalmartScriptedFastPath(
        page, payload, sendLog, sendStatus, tracer, sessionLogger, history, 0,
      );
      if (scripted?.handled) return scripted;
      sendLog("Walmart: Playwright chưa nhận diện được trạng thái hiện tại, chuyển AI fallback.", "warning");
    } catch (error) {
      sendLog(`Walmart: Playwright fast-path lỗi (${error.message}), chuyển AI fallback.`, "warning");
    }
  }
    ...
    for (const toolCall of response.toolCalls) {
      ...
      if (toolResult.terminal) {
        ...
        if (toolResult.terminalAction === "success") {
          ...
          // AI sometimes infers OTP from a failed password submit even though
          // Walmart has not rendered any code field. Validate live DOM first,
          // and also allow the delayed post-login redirect to settle.
          if (isWalmartPayload && toolResult.pauseReason === "otp") {
            await page.waitForTimeout(1200);
            if (await walmartPageIsSignedIn(page)) {
              payload.loginReady = true;
              sendLog("Walmart: Trang đã đăng nhập sau redirect chậm; bỏ qua OTP suy đoán.", "success");
              history.push({
                role: "tool_result",
                toolCallId: toolCall.id,
                toolName: toolCall.name,
                content: JSON.stringify({ terminal: false, skipped: true, reason: "already signed in; OTP pause rejected" }),
                imageBase64: null,
              });
              const scripted = await runWalmartScriptedFastPath(
                page, payload, sendLog, sendStatus, tracer, sessionLogger, history, turn,
              );
              if (scripted?.handled) return scripted;
              hasTerminal = false;
              continue;
            ...
            const realOtpField = await firstVisibleLocator(page, [
            ...
            if (!realOtpField) {
              sendLog("Walmart: DOM chưa có ô OTP; bỏ qua suy luận OTP của AI và tiếp tục login Password.", "warning");
              history.push({
                role: "tool_result",
                toolCallId: toolCall.id,
                toolName: toolCall.name,
                content: JSON.stringify({ terminal: false, skipped: true, reason: "no OTP input exists; continue password login and do not request OTP" }),
                imageBase64: null,
              });
              history.push({
                role: "user",
                content: "Không có ô OTP trong DOM. Không được suy đoán OTP. Hãy dùng input[type=password], xác minh value đã được nhập rồi bấm Sign in; sau đó kiểm tra Hi hoặc Sign Out.",
              });
              hasTerminal = false;
              continue;
            }
      ...
      if (isWalmartPayload && !payload.loginReady && !payload.cartReady && await walmartPageIsSignedIn(page)) {
        ...
        try {
          ...
          if (addrResult.success) {
            ...
            throw new Error(addrResult.reason || "address_sync_failed");
          }
        } catch (e) {
          sendLog(`Walmart: Lỗi sync địa chỉ: ${e.message}`, "warning");
        }
        ...
        try {
          ...
          const clearResult = await walmartPlaybook.clearCart(page);
          if (!clearResult.success || !clearResult.isEmpty) {
            throw new Error(clearResult.reason || "cart_not_empty_after_remove");
          } else if (clearResult.removed > 0) {
            sendLog(`Walmart: Đã xóa ${clearResult.removed} sản phẩm khỏi giỏ hàng.`, "success");
          } else {
            sendLog("Walmart: Giỏ hàng đã trống, không cần xóa.", "info");
        ...
        // Điểm handoff chính: Agentic đã login xong, từ đây Playwright xử lý
        // product/variant/cart/checkout. AI chỉ tiếp tục nếu fast-path không
        // nhận diện được DOM Walmart hiện tại.
        try {
          sendLog("Walmart: Đăng nhập hoàn tất, chuyển sang Playwright fast-path.", "success");
          const scripted = await runWalmartScriptedFastPath(
            page, payload, sendLog, sendStatus, tracer, sessionLogger, history, turn,
          );
          if (scripted?.handled) return scripted;
          sendLog("Walmart: Playwright chưa xử lý hết trạng thái sau login, tiếp tục AI fallback.", "warning");
        } catch (error) {
          sendLog(`Walmart: Playwright sau login lỗi (${error.message}), tiếp tục AI fallback.`, "warning");
        }
      }
