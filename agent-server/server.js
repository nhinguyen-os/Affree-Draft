...
wss.on("connection", async (ws, req) => {
  ...
  function resolveFullViewportFramePoint(xRatio, yRatio) {
  ...
  function shouldStreamCdpFrameDuringPopup(popup = popupFocusState) {
    return Boolean(
      popup
      && popup.kind === "human_verify"
      && popup.mode === "full-viewport"
      && isPremiumOutletsPayload(lastOrderPayload || {})
    );
  }
  ...
  function startPopupFrameInterval() {
    ...
    if (shouldStreamCdpFrameDuringPopup()) return;
  ...
  async function handleClientMessage(messageStr) {
    ...
    try {
      ...
      switch (msg.type) {
        ...
        case "frame_click": {
          if (isAutomating || !page || page.isClosed()) break;
          const xRatio = Math.min(1, Math.max(0, Number(msg.xRatio) || 0));
          const yRatio = Math.min(1, Math.max(0, Number(msg.yRatio) || 0));
          const point = popupFocusState
            ? resolveInteractionFramePoint(popupFocusState, xRatio, yRatio)
            : resolveFullViewportFramePoint(xRatio, yRatio);
          if (!point) break;
          await page.mouse.click(point.x, point.y);
          await page.waitForTimeout(150);
          await sendScreenshotFrame({
            clip: popupFocusState ? point.bounds : undefined,
            mode: `interaction-click-${popupFocusState?.mode || "full-viewport"}`,
          });
          break;
        }
        ...
        case "frame_mouseup": {
          ...
          await sendScreenshotFrame({
            clip: popupFocusState ? point.bounds : undefined,
            mode: `interaction-mouseup-${popupFocusState?.mode || "full-viewport"}`,
          });
          break;
        }
        ...
        case "frame_wheel": {
          ...
          await sendScreenshotFrame({
            clip: popupFocusState ? point.bounds : undefined,
            mode: `interaction-wheel-${popupFocusState?.mode || "full-viewport"}`,
          });
          break;
        }

        case "frame_keypress":
          if (isAutomating || !msg.key || !page || page.isClosed()) break;
          await page.keyboard.press(String(msg.key));
          await page.waitForTimeout(120);
          await sendScreenshotFrame({
            clip: popupFocusState ? getInteractionFrameBounds(popupFocusState) : undefined,
            mode: `interaction-keypress-${popupFocusState?.mode || "full-viewport"}`,
          });
          break;
        ...
        case "coop_payment_screen":
          ...
          try {
            ...
            if (paymentCard) {
              ...
              sendLog("Co.opmart: Chưa nhận đủ thông tin thẻ (số thẻ/expiry/CVV) nên chỉ mở stream để bạn thao tác trực tiếp.", "warning");
            }
  ...
  async function selectCoopVnpayCardGateway(payment) {
    ...
    const hasReachedCardForm = async () => page.evaluate(() => {
      const text = String(document.body?.innerText || "").replace(/\s+/g, " ");
      const hasCardInput = Array.from(document.querySelectorAll("input")).some((input) => {
        const info = [
          input.name,
          input.id,
          input.getAttribute("autocomplete"),
      ...
      return hasCardInput || /số thẻ|so the|card number|cardholder|cvv|cvc|visa|mastercard/i.test(text) && !/chọn phương thức thanh toán/i.test(text);
    }).catch(() => false);

    const clickedInternationalMethod = await page.evaluate(() => {
      ...
      if (!bodyText.includes("chon phuong thuc thanh toan")) return null;
      const rows = Array.from(document.querySelectorAll("a, button, [role='button'], li, div"));
      const scored = rows
        .filter(isVisible)
        .map((el) => {
          const text = normalize(el.textContent || "");
          const hasTarget = text.includes("the thanh toan quoc te") || text.includes("international card");
          if (!hasTarget) return null;
          const rect = el.getBoundingClientRect();
          let score = 1;
          if (rect.width > 260 && rect.height > 45) score += 5;
          if (rect.width > 420 && rect.height < 160) score += 4;
          if (el.matches("a, button, [role='button']")) score += 2;
          if (text.length < 220) score += 2;
          return { el, score, rect: { x: rect.x, y: rect.y, width: rect.width, height: rect.height } };
        })
        .filter(Boolean)
    ...
    if (clickedInternationalMethod) {
      await page.mouse.click(
        clickedInternationalMethod.x + clickedInternationalMethod.width / 2,
        clickedInternationalMethod.y + clickedInternationalMethod.height / 2,
      ).catch(() => { });
      await page.waitForLoadState("domcontentloaded", { timeout: 15000 }).catch(() => { });
      await page.waitForTimeout(1500).catch(() => { });
      await sendScreenshotFrame({ mode: "coop-vnpay-selected-international-card" }).catch(() => { });
      let reachedCardForm = await hasReachedCardForm();
      if (!reachedCardForm) {
        const retryPoints = [
          { x: clickedInternationalMethod.x + clickedInternationalMethod.width - 48, y: clickedInternationalMethod.y + clickedInternationalMethod.height / 2 },
          { x: clickedInternationalMethod.x + Math.min(180, clickedInternationalMethod.width * 0.22), y: clickedInternationalMethod.y + clickedInternationalMethod.height / 2 },
          { x: clickedInternationalMethod.x + clickedInternationalMethod.width * 0.78, y: clickedInternationalMethod.y + clickedInternationalMethod.height / 2 },
          { x: clickedInternationalMethod.x + clickedInternationalMethod.width / 2, y: clickedInternationalMethod.y + clickedInternationalMethod.height * 0.72 },
        ];
        for (const point of retryPoints) {
          await page.mouse.click(point.x, point.y).catch(() => { });
          await page.waitForLoadState("domcontentloaded", { timeout: 8000 }).catch(() => { });
          await page.waitForTimeout(900).catch(() => { });
          reachedCardForm = await hasReachedCardForm();
          if (reachedCardForm) break;
        }
        await sendScreenshotFrame({ mode: "coop-vnpay-selected-international-card-retry" }).catch(() => { });
      }
      if (reachedCardForm) {
        return {
          selected: true,
          brand: selectedBrand,
          brandLabel: "Thẻ thanh toán quốc tế",
        };
      }
    }
  ...
  async function startScreencastForCurrentPage() {
    ...
    cdpSession.on("Page.screencastFrame", ({ data, sessionId }) => {
      try {
        const streamDuringPopup = shouldStreamCdpFrameDuringPopup();
        if (!popupFocusState || streamDuringPopup) {
          const viewportBounds = streamDuringPopup ? getCurrentViewportBounds() : null;
          ws.send(JSON.stringify({
            type: "screencast",
            data: data,
            width: 1024,
            height: 768,
            mode: streamDuringPopup ? "premiumoutlets-human-verify-realtime" : "default",
            sourceX: viewportBounds?.sourceX ?? 0,
            sourceY: viewportBounds?.sourceY ?? 0,
            sourceWidth: viewportBounds?.width,
            sourceHeight: viewportBounds?.height,
          }));
        }
      } catch (err) { }
