...
#   - Google Chrome thật đã được cài đặt (npx playwright install chrome) — cần
#     cho phiên Walmart chạy headed bằng channel "chrome"
...
# Phiên Walmart luôn ép dùng Google Chrome thật (channel "chrome" trong
# server.js), không phải Chromium mặc định của Playwright, nên cần cài riêng.
if ! find "$HOME/.cache/ms-playwright" /opt/google/chrome -maxdepth 2 -iname "chrome" 2>/dev/null | grep -q .; then
  echo "⚠ Chưa tìm thấy Google Chrome thật (channel chrome). Đang cài đặt..."
  npx playwright install chrome
  echo "✓ Đã cài đặt Google Chrome."
fi
