...
async function handleAddressPopup(page, payload, sendLog, sendMessage) {
  ...
  const address = page.locator('span:has-text("Giao đến:")').first();
  if (await waitForVisible(address, 5000)) {
    sendLog("Bach Hoa Xanh: Địa chỉ giao hàng đã được chọn.", "success");
  }
...
async function checkout(page, payload, sendLog, sendStatus, sendMessage) {
  ...
  for (const sel of buySelectors) {
    try {
      ...
      if (await waitForVisible(btn, 10000)) {
        buyBtn = btn;
  ...
  if (!buyBtn) {
    ...
    const cartSelectors = [
      'span:has-text("Giỏ hàng")',
    ];
    for (const sel of cartSelectors) {
      try {
        const btn = page.locator(sel).first();
        if (await element.isVisible()) {
          sendLog("Bach Hoa Xanh: Đang trong giỏ hàng.", "warning");
          break;
        }
      } catch {}
    }
    ...
    const emptySelectors = [
      'span:has-text("Giỏ hàng của Bạn")',
      'a:has-text("Tiếp tục mua hàng")',
    ];
    for (const sel of emptySelectors) {
      try {
        const btn = page.locator(sel).first();
        if (await element.isVisible()) {
          sendLog("Bach Hoa Xanh: Giỏ hàng trống, không thể đặt hàng.", "warning");
          break;
        }
      } catch {}
    }
  ...
  if (paymentMethod) {
    sendMessage("Chọn phương thức thanh toán.");

    const paymentSelectors = [];
    if (paymentMethod === 'cod') {
      paymentSelectors.push('p:has-text("Thanh toán khi nhận hàng")');
    } else if (paymentMethod === 'card' && payload.cardInfo) {
      paymentSelectors.push('p:has-text("Thẻ Visa, Master, JCB")');
    } else {
      paymentSelectors.push('p:has-text("Chuyển khoản")');
    }
    for (const sel of paymentSelectors) {
      try {
        ...
        if (await waitForVisible(option, 1000)) {
          ...
          sendLog(`Bach Hoa Xanh: Đã chọn phương thức thanh toán: ${sel}`, "success");
        } else {
          const selectChange = page.locator('button:has-text("Đổi")').first();
          if (await waitForVisible(selectChange, 1500)) {
            await selectChange.click();
            await page.waitForTimeout(200);
