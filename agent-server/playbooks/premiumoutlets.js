  ...
  for (const selector of iframeSelectors) {
    try {
      ...
      for (let index = 0; index < count; index += 1) {
        const handle = await locators.nth(index).elementHandle();
...
async function fillFirstVisibleInput(page, selectors, value, options = {}) {
  ...
  for (const frame of frameContexts) {
    if (await fillVisibleInputInContext(frame, selectors, text)) {
      return true;
    }
  }
...
async function chooseShopSimonCardPayment(page, sendLog) {
  ...
  for (const selector of selectors) {
    try {
      ...
      sendLog("Premium Outlets: đã chọn credit/debit card trên Shop Simon checkout.", "info", { audience: "client" });
      return true;
    } catch {}
  }
...
async function fillShopSimonPaymentCard(page, payload, sendLog, sendStatus) {
  ...
  if (!card || card.number.length < 12 || !card.expMonth || !card.expYear || card.cvv.length < 3) {
    sendStatus("waiting_user_input", {
      reason: "Premium Outlets: thiếu card payload usable để auto-fill payment trên Shop Simon checkout.",
  ...
  if (!cardNumberFilled) {
    sendStatus("waiting_user_input", {
      reason: "Premium Outlets: chưa detect được ô số thẻ trên Shop Simon checkout để auto-fill.",
      requiredInput: "final_confirmation",
      orderUrl: page.url(),
    });
  ...
  if (!combinedExpiryFilled) {
    ...
    if (!monthFilled || !yearFilled) {
      sendStatus("waiting_user_input", {
        reason: "Premium Outlets: chưa detect được ô hạn thẻ trên Shop Simon checkout để auto-fill.",
        requiredInput: "final_confirmation",
        orderUrl: page.url(),
      });
  ...
  if (!cvvFilled) {
    sendStatus("waiting_user_input", {
      reason: "Premium Outlets: chưa detect được ô CVV trên Shop Simon checkout để auto-fill.",
      requiredInput: "final_confirmation",
      orderUrl: page.url(),
    });
  ...
  sendLog(`Premium Outlets: đã auto-fill card Shop Simon với thẻ ****${card.number.slice(-4)}${nameFilled ? " và cardholder name" : ""}.`, "success", { audience: "client" });
...
async function ensureShopSimonProductPage(page, productName, sendLog, sendStatus) {
  ...
  sendStatus("running", {
    reason: `Premium Outlets: đã land tại Shop Simon product detail${headingText ? ` — ${headingText}` : ""}.`,
  });
  return true;
}
...
async function clickFirstVisible(page, selectors, sendLog, options = {}) {
  ...
  for (const selector of selectors) {
    try {
      ...
      sendLog(`${successMessage}: ${selector}`, "success", { audience: "client" });
      return true;
    } catch {}
  }
...
async function setShopSimonQuantity(page, qty, sendLog) {
  ...
  for (const selector of SHOP_SIMON_QUANTITY_INPUT_SELECTORS) {
    try {
      ...
      sendLog(`Premium Outlets: đã thử cập nhật số lượng Shop Simon lên ${qty}.`, "info", { audience: "client" });
      return true;
    } catch {}
  }
...
async function gotoShopSimonCart(page, sendLog) {
  const openedBag = await clickFirstVisible(page, SHOP_SIMON_VIEW_BAG_SELECTORS, sendLog, {
    successMessage: "Premium Outlets: đã mở bag/cart Shop Simon",
    failureMessage: "Premium Outlets: chưa thấy nút View Bag, thử mở cart trực tiếp.",
  });
  ...
  sendLog(`Premium Outlets: đã vào Shop Simon cart (${currentUrl}).`, "success", { audience: "client" });
  return true;
}
...
async function gotoShopSimonCheckout(page, sendLog) {
  ...
  if (!onCheckout) {
    ...
    if (!checkoutHeading) {
      sendLog(`Premium Outlets: đã bấm checkout nhưng chưa xác nhận được checkout page (${checkoutUrl}).`, "warning", { audience: "client" });
      return false;
    }
  }

  sendLog(`Premium Outlets: đã tới Shop Simon checkout (${checkoutUrl}).`, "success", { audience: "client" });
  return true;
}
...
async function fillShopSimonCheckout(page, payload, sendLog, sendStatus) {
  ...
  if (!buyerEmail) {
    sendStatus("waiting_user_input", {
      reason: "Premium Outlets: đã tới Shop Simon checkout nhưng request chưa có email để điền contact.",
      requiredInput: "final_confirmation",
      orderUrl: page.url(),
    });
  ...
  if (!parsedAddress) {
    sendStatus("waiting_user_input", {
      reason:
        "Premium Outlets: đã tới Shop Simon checkout nhưng địa chỉ chưa ở định dạng US đủ rõ (ví dụ: street, city, ST ZIP) để auto-fill shipping.",
      requiredInput: "final_confirmation",
      orderUrl: page.url(),
    });
  ...
  for (const field of fields) {
    ...
    if (!filled) {
      sendStatus("waiting_user_input", {
        reason: `Premium Outlets: tới Shop Simon checkout nhưng chưa tự điền được trường ${field.label}.`,
        requiredInput: "final_confirmation",
        orderUrl: page.url(),
      });
  ...
  if (!stateSelected) {
    sendStatus("waiting_user_input", {
      reason: `Premium Outlets: đã điền phần lớn checkout Shop Simon nhưng chưa chọn được bang ${parsedAddress.stateCode}.`,
      requiredInput: "final_confirmation",
      orderUrl: page.url(),
    });
  ...
  sendStatus("waiting_user_input", {
    reason: `Premium Outlets: contact / delivery + card đã sẵn sàng trên Shop Simon${cardAutofill.last4 ? ` (****${cardAutofill.last4})` : ""}. Kiểm tra lại rồi bấm xác nhận bước cuối trên website thật.`,
    requiredInput: "final_confirmation",
    orderUrl: page.url(),
  });
...
async function runShopSimonBranch(page, payload, sendLog, sendStatus, outbound) {
  ...
  const addedToBag = await clickFirstVisible(page, SHOP_SIMON_ADD_TO_BAG_SELECTORS, sendLog, {
    timeout: 2200,
    clickTimeout: 4000,
    successMessage: "Premium Outlets: đã thêm sản phẩm vào Shop Simon bag",
    failureMessage: "Premium Outlets: chưa thấy nút Add to bag trên Shop Simon product page.",
  });
  if (!addedToBag) {
    sendStatus("waiting_user_input", {
      reason: "Premium Outlets: đã vào Shop Simon product page nhưng không tự thêm được vào bag; cần người dùng tiếp tục thủ công.",
      requiredInput: "final_confirmation",
      orderUrl: page.url(),
    });
    return { done: true };
  ...
  await page.waitForTimeout(1500).catch(() => {});
  sendStatus("running", {
    reason: "Premium Outlets: Shop Simon đã add to bag, đang chuyển sang cart.",
  });
  ...
  sendStatus("running", {
    reason: quantityReady
      ? "Premium Outlets: đã vào Shop Simon cart, đang thử tiến tới checkout."
      : "Premium Outlets: đã vào Shop Simon cart nhưng số lượng cần người dùng kiểm tra lại trước khi checkout.",
  });
...
async function continueAfterVerify(page, payload, sendLog, sendStatus) {
  ...
  if (blockedState) {
    sendLog(
      "Premium Outlets: human verification vẫn còn hiển thị sau khi người dùng thả chuột; tiếp tục giữ verify trên browser.",
      "warning",
      { audience: "client" }
    );
    sendStatus("waiting_user_input", {
      reason: `${blockedState.reason} Human verification vẫn còn hiển thị. Hãy giữ tiếp cho tới khi widget pass rồi mới thả chuột.`,
  ...
  sendLog(
    "Premium Outlets: human verification đã qua; tiếp tục đọc lại BUY NOW và checkout theo playbook thay vì nhảy ngay sang agentic loop.",
    "success",
    { audience: "client" }
  );
  ...
  sendStatus("running", {
    reason: `Premium Outlets: verify xong, tiếp tục BUY NOW → ${describeDestination(outbound)}.`,
  });
...
async function run(page, payload, sendLog, sendStatus) {
  ...
  sendLog(`Premium Outlets: bắt đầu bootstrap cho ${productName || "sản phẩm"}.`, "info", { audience: "client" });
  ...
  if (blockedState) {
    sendLog(
      "Premium Outlets: gặp màn hình anti-bot / human verification trước CTA BUY NOW; tạm dừng để user giữ nút xác minh trong browser.",
      "warning",
      { audience: "client" }
    );
    sendStatus("waiting_user_input", {
      reason: `${blockedState.reason} Vui lòng hoàn tất human verification trên browser rồi bấm tiếp tục phiên.`,
      requiredInput: "captcha",
      pauseReason: "premiumoutlets_verify",
      orderUrl: page.url(),
    });
