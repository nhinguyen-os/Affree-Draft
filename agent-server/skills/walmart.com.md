...
- Nếu gặp OTP/CAPTCHA hoặc bước review thanh toán, dừng bằng `pause_for_human(...)`.
- Chỉ sau khi người dùng bấm `Xác nhận đặt hàng` trên popup Affree, server mới nhận
  `walmart_confirm_order`, điền thẻ và bấm nút đặt hàng cuối. Qwen không tự bấm nút cuối.
...
10. Sau khi xác minh đúng sản phẩm đã vào giỏ, mở checkout bằng `Buy now`, `View cart` hoặc `Checkout` rồi dừng bằng `pause_for_human(reason="review")`; popup Affree sẽ yêu cầu người dùng kiểm tra thẻ/CVV và xác nhận trước khi server hoàn tất đơn.
...
5. Qwen không tự nhập dữ liệu thẻ và không tự bấm nút tạo đơn.
6. Payment helper chỉ được điền thẻ và bấm `Place order` sau event `walmart_confirm_order` phát sinh từ thao tác xác nhận rõ ràng của người dùng trên popup Affree.
...
- Dùng Agentic cho toàn bộ giai đoạn login (email, Password, OTP/CAPTCHA nếu Walmart yêu cầu).
- Ngay sau khi xác nhận signed-in, bàn giao sang Playwright deterministic cho popup đã biết,
  địa chỉ, variant, add cart, checkout và payment. Chỉ gọi lại Qwen khi selector/playbook
  sau login trả trạng thái không nhận diện được.
- Các modal đã biết như `Remove address?` phải xử lý bằng role/text/data-intent trong
  `handleKnownModal()`, không gửi ảnh/DOM cho AI.
- Helper DOM nhận biết lúc cần Affree form hoặc stream; AI là fallback cuối cùng.
